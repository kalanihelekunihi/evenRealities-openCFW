...
| Deployment preservation and recovery existed only as prose | Closed offline: the bundle embeds a machine-checked no-mass-erase contract; the preflight requires exact 1-MiB internal-flash and complete 0x308-byte architected nRF52840 UICR backups, accepts only exact two-data/one-swap retail FDS settings or fully erased pages, erases the complete source-built boot/application/settings extent so no opaque retail bytes or incompatible credentials survive, preserves only product data, and emits separate canonical recovery HEX files plus the exact expected full readback; the dry-run-default SWD executor requires exact bundle and unique probe identities, directly drives transparent NVMC sector/word operations, withholds reset until independent readback verification, restores the exact retail flash in normal rollback without touching UICR, and exposes a separately confirmed ERASEUICR-based disaster path that restores and verifies the exact UICR backup without ERASEALL | `tools/prepare_zephyr_deployment.py`; `tools/verify_zephyr_deployment.py`; `tools/deploy_zephyr_swd.py`; `tools/verify_zephyr_bundle.py` |
...
normal full-flash rollback, separately confirmed UICR erase/restore, and
post-recovery comparison. The remaining deployment criteria—actual flash, boot,
rollback, recovery, and power-loss validation—cannot be truthfully closed until
the authorized ring is connected through a uniquely identified SWD probe.
