...
Nine resolved application entries are single-instruction Thumb-2 `B.W` thunks to
...
| `0x0007D2A0..<0x0007D2A4` | `0x000987C4` | authenticated FreeRTOS 10.5.1 `xTaskGetTickCount` | none | `2e747c4b74eed771e0d3c7c91da71d5bf94b6dc460a9a4ed5ef5a33812362886` |
...
The closure is 9 functions / 36 bytes. The first thunk remains inside the licensed Goodix gate;
the second is only an alias of an admitted R1 board adapter; and the six health/activity thunks are
aliases of already implemented product-owned metadata operations. The additional
`0x0007D2A0` seed is a manual provenance supplement because the curated
flash-boundary script described a 2.2.7 NVMC helper while the preserved 2.2.6
image actually contains a direct alias to its exact `xTaskGetTickCount` at
`0x000987C4`. The authenticated FreeRTOS source supplies that target; no
third-party implementation is copied or reconstructed.
