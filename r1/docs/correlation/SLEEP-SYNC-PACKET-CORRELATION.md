...
`0x0008F9C2`. `scripts/firmware/summarize_r1_sleep_sync_packet.py` authenticates all three facts and
...
```sh
python3 scripts/firmware/summarize_r1_sleep_sync_packet.py
make -C openR1 test
```
