...
Five extents omitted by Ghidra's function inventory are manual provenance supplements with
unambiguous prologue, return, caller, and adjacent-data boundaries. The verifier hashes the exact
bytes for all fifteen entries. The shared accumulator is product bookkeeping rather than a signal
...
`r1_stress_store_sample`, `r1_temperature_cache_read_slot`, and
