...
|---|---:|---|
| `[0x00439710,0x004397A6)` | 150 | Opaque authenticated `__aeabi_memmove` provider; backward paths and non-overlap tail to `memcpy` audited |
| `[0x00439BE4,0x00439C8A)` | 166 | Opaque authenticated `__aeabi_memcpy` provider, including alternate interior entry `0x00439C04` |
...
## Current Apollo-main nanopb skip-string map
...
manifest and both current profiles are offline build evidence, not hardware
authorization.
