...
1. The runtime-backed `kvbooCount` default at `0x20074988` still has no
   authenticated initial value or lifecycle semantics.
2. There is no authenticated, non-mutating golden capture of the G2 `kvdb`
...
3. The Even application schemas and factory calibration semantics are not
   supplied by upstream FlashDB.
4. Upstream `fdb_kvdb_init` can reach corruption/magic-driven formatting and
