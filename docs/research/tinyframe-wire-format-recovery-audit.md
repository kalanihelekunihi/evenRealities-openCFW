...
gap in [`docs/upstream-inventory.md`](../upstream-inventory.md) — ID/length/
...
## Remaining focused checks before vendoring
...
2. Recover the send-side (`TF_Compose*`, peer bit and `next_id` rollover) to
   pin the transmit format and the `TF_ID` peer-bit policy; the receive path
   above already fixes the wire widths and checksum.
...
4. Pin the exact TinyFrame revision (the API surface and the non-standard
   `TFState` numbering suggest a lightly patched vendor copy); vendor `pb.h`-
   style under MIT with G2 `TF_Config.h` in a separate port layer.
