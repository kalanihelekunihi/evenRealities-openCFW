...
This tranche also closes the two default-table descriptors and all initialized
default bytes except the runtime-backed `kvbooCount` word. It recovers the
database mutex callbacks, the exact NOR callback return conventions, and the
first-party magic/reset policy. These are static facts, not permission to run
the destructive policy on hardware.
...
| `kvbooCount` | 4 | unresolved: value pointer `0x20074988` is outside the decoded initialized-data record |
...
The complete descriptor functions, table-consuming validators, IAR record,
decoded output hash, keys, pointers, lengths, and initialized bytes are all
mutation tested. The only default byte value not statically authenticated is
`kvbooCount`; treating it as zero merely because it is in a high RAM region
would be an assumption, so the analyzer refuses to do so.
...
3. Represent the recovered default-KV tables in openCFW-owned schema code;
   keep the unresolved `kvbooCount` value lifecycle and all Even `NV`/`db_api`
   policy outside the upstream directory.
