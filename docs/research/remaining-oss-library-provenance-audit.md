...
stock default/reset behavior; the runtime `kvbooCount` default and higher
level schema semantics also remain unresolved.
...
Focused disassembly still needs to answer or finish:

- TrueType interpreter default (v35 versus v40);
- exact GX variation support behind the multi-masters service;
- the remaining `FT_Done_FreeType` lifecycle and caller closure
  (`FT_Done_Face` is proven exactly at `[0x00526814,0x0052687E)`);
- the stock memory callbacks reached through `FT_New_Memory` and their heap,
  alignment, failure, and ownership policy;
- target `FT_CONFIG_OPTION_*` header generation and module-table bytes;
- complete entry/interior/stored-pointer closure for each admitted API; and
- comparison of LVGL glyph/face/cache behavior against an official-source
  host oracle using the actual G2 font assets.
...
|---|---|---|
| LVGL | eight mapped core bodies and configuration/ABI discriminators establish official-history compatibility interval `60d976c466e8…344c7c318047`; the released v9.3.0 tag is excluded as too new; 78 paths split into upstream core/FreeType wrappers, 11 Ambiq draw files, and separate vendor/Even glue | exact vendor commit/tree remains unresolved inside the interval and Ambiq/display/input/FreeType-system layers are not upstream LVGL; see `lvgl-version-recovery-audit.md` |
...
| TinyFrame | full send topology and receive anchors establish MIT upstream source-equivalence interval `44ecc068…a29167a6`; SOF/width/endian/CRC, 1024-byte TX buffer, soft lock, request/response peer-ID policy, listener-timeout ABI, and transport seam are recovered | exact vendor commit remains unresolved because G2 changes object magic/layout/logging/transport; the production-excluded parser candidate now includes SOF in HEAD_CKSUM, omits DATA_CKSUM for zero length, and has regression coverage, but still awaits source ownership of the surrounding G2 object/transport ABI; see `tinyframe-send-version-recovery-audit.md` |
