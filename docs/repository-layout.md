...
| `tools/` | host simulator and the SDK-image verifier |
...
- **R1 raw BSim output.** The correlation CSVs and Ghidra logs behind
  `r1/docs/reference/bsim/` are regenerable and carry machine-local paths; the
  summaries keep their input and output hashes so a fresh run can be checked
  against the recorded one. (G2 is different: its Ghidra output *is* kept, under
  `g2/research/corpus/`, because analyzers hash-check it.)
- **The upstream research workspace.** R1 evidence documents cite analysis
  scripts by their `scripts/firmware/...` path in the research workspace where
  the evidence was produced. Those citations are provenance, and are left as
  written; the scripts are not part of this repository. See
  [`../r1/docs/README.md`](../r1/docs/README.md).
