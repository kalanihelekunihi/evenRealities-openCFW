...
| DaveGamble cJSON | Version interval v1.7.9–v1.7.12 from four binary discriminators: the ≥1.7.9 issue-#315 `get_object_item` fix, the <1.7.13 `buffer_skip_whitespace` offset behavior, the absent <1.7.14 `parse_array`/`parse_object` `head->prev` tail store, and the <1.7.19 64-byte stack `parse_number` buffer | **Identified, not yet source-admitted** — no vendored snapshot exists and no representative tag/commit inside the interval is selected. The parser shared by `service_android_notify.c` and `service_whitelist.c` is bounded as 21 functions / 2,572 body bytes at `[0x004D798C,0x004D83D8)` with 34 external caller sites. The remaining gate is an explicit source-admission decision, not identification work. See the [source-candidate audit](research/g2-json-parser-source-candidate-audit.md) |
...
DaveGamble cJSON v1.7.9–v1.7.12 (identified; not yet source-admitted), and
