...
The narrower authenticated function/source correlation is implemented by
[`../../tools/analyze_g2_lvgl_source_mapping.py`](../../tools/analyze_g2_lvgl_source_mapping.py)
and remains production-excluded.
...
The authenticated 64-shard corpus adds one deliberately narrow source mapping:
`lv_iter_create` is the complete 178-byte function
`0x005C3B00..0x005C3BB2` (SHA-256
`847ee3dfb073799b7aeb960e5ae8bdfbcc245808186043d882931bc78f81655f`).
Its body alone references the retained `src/misc/lv_iter.c` path through
pointer cell `0x005C3BC4` to string `0x006E8794`. The decompilation pins the
28-byte iterator allocation, optional context allocation, fields at offsets
0/4/8/12/24, and assertion/log line constants 65, 68, and 79. Those facts
match vendored ceiling blob `cb6377c0b50060710e5446dce5d083bfe3bed539`.

This is function ownership evidence only. The closed corpus contains no direct
caller token, but that does not exclude indirect reachability. Original
compiler relocation records and complete caller closure are absent, so no
candidate source or production-overlay entry is added. Adjacent functions are
not assigned to `lv_iter.c` merely because the string is nearby.
...
| `LV_USE_SPAN` | 1 | retained `lv_span_stack_init` and span state at `lv_global+0x1C4` |
| `LV_USE_OBJ_ID_BUILTIN` | 1 | object-ID array/count required at stock `lv_global+0x1C8/+0x1CC` |
| `LV_DRAW_SW_COMPLEX` | 0 | the stock global omits the 112-byte software circle-mask cache and standard software-draw units are absent |
| `LV_USE_STDLIB_MALLOC` | `LV_STDLIB_CUSTOM` | the stock global omits the 32-byte built-in TLSF state and `lv_init` has no built-in memory initializer |
...
task-notification selection, custom allocator-hook identities, every widget macro, or unused
...
unconfigured official checkout a safe drop-in. As an informative negative reference
...
`lv_global_t == 0x1F8`, not G2's `0x1EC`. The discrepancy is now completely
explained: G2 disables the 112-byte complex-mask cache, omits the 32-byte
built-in TLSF state through custom allocation, and adds 24 bytes through two
Ambiq callbacks in each of three draw-buffer handler tables. Applying the
recovered Ambiq patch and proven feature set reproduces `0x1EC` and every
observed internal offset. Without short enums the same headers produce
`lv_display_t == 0x324` and `lv_indev_t == 0xE8`. The ABI gate is closed. The
public GPU dependency is now identified as AmbiqSuite 5.1.0, NemaGFX 1.4.12,
NemaVG 1.1.8, exact subtree `e690768a…` at reproduction commit `b853fded…`,
with stock command-list and power policy recovered. All 11 linked GPU-patch
exports and all 18 linked bare-metal HAL functions now have bounded clean-room
source candidates. Production integration remains fail-closed on candidate
admission, atomic integration, stock-IAR ABI confirmation, and hardware gates.
...
| Ambiq draw backend inside `LVGL/src/draw/ambiq` | 11 | exact public Ambiq subtree `1e774257…`; separate from official LVGL history |
...
The eleven linked Ambiq draw files include the backend root plus arc, buffer, fill,
image, letter, mask-rect, triangle, box-shadow, private, and vector-font
translation units. No official interval/tag tree contains those paths. Their
exact public source is now recovered at Ambiq subtree tree `1e774257…`, with
canonical commit `5be8e0ae…` and byte-identical replay `67fd93e2…`; see
[`lvgl-ambiq-source-abi-recovery-audit.md`](lvgl-ambiq-source-abi-recovery-audit.md).
...
3. Preserve the recovered ABI configuration and assert it for the production
   compiler/short-enum settings.
4. Import the authenticated Ambiq subtree and exact public Nema package
   atomically only after the stock-IAR/binary-admission decision, bare-metal
   HAL/IRQ/allocator port, and target-validation gates close; keep
   the already source-owned `lv_ambiq_display.c` compatibility port,
   `am_ftsystem.c`,
...
The stock bare-metal Nema HAL/IRQ/allocator and separate Ambiq display port are
now behaviorally bounded; the latter is already production source-owned. The
remaining useful work is candidate admission/atomic integration, hardware
validation, configuration/assets, and first-party manager recovery, not
decompiling the identified Ambiq, Nema-package, or official LVGL algorithms.

## Remaining unknowns

- Exact whole Even/Ambiq hybrid-tree commit; the Ambiq backend subtree itself is exact.
...
- Original IAR Nema archive/private GPU-patch and bare-metal HAL source commit.
  Their complete linked G2 surfaces have bounded clean-room candidates, but
  still require source admission, atomic integration, and hardware validation.
- Original private `lv_ambiq_display.c` source/commit provenance. Its complete
  seven-function / 638-byte linked surface is already source-owned, so this is
  historical attribution rather than a functional gap.
