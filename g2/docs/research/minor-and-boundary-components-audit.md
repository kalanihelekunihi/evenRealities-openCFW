...
## `third_party\ringBuffer\ringbuffer.c` — AndersKaloer/Ring-Buffer

| Evidence | Run address |
|---|---|
| Vendored path `third_party\ringBuffer\ringbuffer.c` | `0x006FC92C` |
| Exact expanded power-of-two assertion | `0x0074D6EC` |
| Complete function cluster | `[0x00598134,0x0059823C)` |

The path alone was not discriminating, but the complete cluster is. Its
runtime `buf_size` parameter, exact assertion expansion, 16-byte dynamic
object, seven-function API, and overwrite-oldest policy match the dynamic
AndersKaloer/Ring-Buffer change introduced by `cda00e1`. openCFW selects
source-equivalent pre-build commit `190e30b` while preserving the honest
`cda00e1…190e30b` vendor-checkout interval. See
`ring-buffer-lineage-recovery-audit.md`; the seven-function closure is now
production-integrated under Apple Clang, with Linux profile replay pending.
