...
- select
  [`../../third_party/freertos-kernel/portable/IAR/ARM_CM55_NTZ/non_secure`](../../third_party/freertos-kernel/portable/IAR/ARM_CM55_NTZ/non_secure),
...
This complete setup routine now has a separate production-excluded clean-room
candidate with Apple/Linux target bodies and all relocations pinned. Its
Ambiq constants are authenticated against the selected 5.1.0 snapshot; no
production or hardware state is changed.
...
The elapsed-tick/IRQ pair and tickless function now each have separate
production-excluded clean-room candidates with deterministic host oracles and
Apple/Linux object/relocation pins. This closes the bounded STIMER algorithms,
not the first-party power-hook policy or device validation.
...
These seams are now complete-object closed in
[`g2-product-rtos-recovery.md`](g2-product-rtos-recovery.md). The surrounding
13-function object adds a 32-slot task-vote policy and proves that the pre-
sleep hook selects deep sleep only when the active-vote count is zero; pre-
sleep, post-sleep, and idle each feed the watchdog. Its complete stock
`am_hal_sysctrl_sleep` provider contains the two-WFI internal-timer retry path
from the public Apollo510 HAL 5.1.0 lineage, excluding 5.0.0. Public commit
`5efc0228528a8adce5eae0d226fac85d2551eb3b` is the selected source replay.
The firmware build predates that public import, so the exact private
pre-release generating commit remains unobservable.
...
2. apply and verify the recovered
   `components/shared/freertos/g2-tcb-v10.5.1.patch`, which implements the G2
   stack-depth extension at `+0x54` and mirrors it in `StaticTask_t`;
3. provide the now-source-qualified STIMER compare-A IRQ 32 setup, ISR, and
   tickless functions, plus the remaining first-party power hooks;
...
Gate 2 is now closed at the source/ABI boundary. The 1,730-byte patch applies
cleanly to authenticated V10.5.1, compiles the public static TCB to `0x70`,
and pins every shifted offset. See
`freertos-g2-tcb-vendor-patch-audit.md`. It does not close the independent
global-placement, hardware-timing, hook, or complete-configuration gates.
The bounded STIMER and `vTaskSwitchContext` trace-ring algorithms now have
separate production-excluded dual-profile candidates; that source
qualification does not itself close their atomic production-integration or
...
extension, external trace behavior, Apollo STIMER tick/tickless code, timer
static memory, and `heap_4` placement are mandatory compatibility work. The
application hooks are now behaviorally closed but not production-routed. MVE,
the exact private HAL pre-release commit, remaining `INCLUDE_*` switches, and
device-wide secure attribution remain explicit follow-up items.
