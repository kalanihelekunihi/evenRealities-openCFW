...
`tools/analyze_g2_kvdb_universal_setting.py` now validates the production routing
...
exactly; the report now states `production_routed: true` with 340
ownership bytes and 48 retained stock tail bytes, matching the ownership
