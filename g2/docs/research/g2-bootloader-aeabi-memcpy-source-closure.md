...
`make bootloader-aeabi-memcpy-closure` rebuilds the source package and then checks host semantics, the authenticated stock span, all 33 callers, strict leaf identity, placement, provider accounting, manifest ownership, and both deployment profiles. No closure tool signs, transmits, flashes, resets, or communicates with hardware.
