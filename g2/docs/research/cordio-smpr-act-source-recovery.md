...
`components/apollo_main/core_overlay/cordio_smpr_act.c` is now the production
adapter for all ten linked actions. Ten independently compiled, strict-relocation
leaves contribute 838 C-generated bytes plus fourteen alignment bytes, and ten
authenticated `b.w` entry replacements replace all 1,160 stock function bytes.
The 44 stock non-code bytes remain official data. Host tests cover the security
request, pairing-request indication, response construction and SC/legacy
selection, confirm calculation and failure handling, the r20 `keyReady`
transition, random transmission, and both key-distribution directions.

Functional validation on an authorized G2/EM9305 remains explicitly blocked:
no physical responder legacy-pairing, confirm/random, STK handling,
