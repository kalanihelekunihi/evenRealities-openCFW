...
`tools/analyze_g2_kvdb_onboarding_config.py` now validates the production routing
...
exactly; the report now states `production_routed: true` with 286
ownership bytes and 54 retained stock tail bytes, matching the ownership
