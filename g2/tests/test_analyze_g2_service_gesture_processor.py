import importlib.util
import sys
import unittest
from pathlib import Path

P = Path(__file__).resolve().parents[1] / "tools/analyze_g2_service_gesture_processor.py"
S = importlib.util.spec_from_file_location("g2_service_gesture_processor", P)
M = importlib.util.module_from_spec(S)
sys.modules[S.name] = M
S.loader.exec_module(M)


class ZeroAnchorClosureTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.r = M.analyze()

    def test_identity(self):
        self.assertEqual(self.r["identity"], {'disposition': 'linked-unanchored', 'ghidra_discovered_functions': 0, 'image_sha256': '36c5b0e499a68ac2493a497bdab9740fd3e7027730c26a9094eca47268a27863', 'path_anchored_functions': 0, 'retained_path': 'platform\\input\\service_gesture_processor.c', 'retained_product_path': 'D:\\01_workspace\\s200_ap510b_iar_git\\platform\\input\\service_gesture_processor.c'})

    def test_surface(self):
        self.assertEqual(self.r["surface"], {'body_bytes': 1236, 'direct_body_calls': 68, 'function_escapes': 0, 'indirect_body_calls': 0, 'internal_direct_body_calls': 6, 'linked_functions': 5, 'outer_pool_bytes': 110, 'path_literal_references': 5, 'physical_bytes': 1346, 'raw_path_referencing_functions': 2, 'reachable_instructions': 501})

    def test_ingress(self):
        self.assertEqual(self.r["ingress"], {'direct_b16_entry_sites': 0, 'direct_bl_entry_sites': 11, 'direct_bl_strict_interior_sites': 1, 'direct_bw_entry_sites': 0, 'stored_entry_pointer_words': 0})

    def test_evidence_and_production(self):
        self.assertEqual(self.r["evidence"], {'boundary_guards': True, 'pointer_cells': ['0x00503238'], 'path_string_run_address': '0x006EF958', 'tag_strings': 5})
        self.assertEqual(
            self.r["production"],
            {
                "candidate": "components/apollo_main/core_overlay/service_gesture_processor.c",
                "production_routed": True,
                "ownership_bytes": 2954,
                "source_inventory_available": True,
                "source_functions": 5,
                "compiled_text_bytes": 1608,
                "alignment_bytes": 6,
                "stock_replaced_bytes": 1346,
                "strict_relocations": 53,
                "software_functional_gap": False,
                "hardware_validation": "deferred by project direction",
                "hardware_blocker": (
                    "Authorized physical G2 touch/proximity device or captured "
                    "gesture electrical/event/timing evidence is required for future qualification."
                ),
            },
        )


if __name__ == "__main__":
    unittest.main()
