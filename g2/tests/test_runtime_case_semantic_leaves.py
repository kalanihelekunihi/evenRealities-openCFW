...
FIXTURE = ROOT / "tests/fixtures/runtime_case_semantic_leaves_host.c"


class CaseSemanticLeavesTests(unittest.TestCase):
    def test_host_oracle(self) -> None:
        clang = shutil.which("clang")
        if clang is None:
            self.skipTest("clang unavailable")
        with tempfile.TemporaryDirectory(prefix="g2-case-leaves-host-") as tmp:
    ...
    def test_strict_cortex_m0plus_compile(self) -> None:
        ...
        if clang is None:
            self.skipTest("clang unavailable")
        with tempfile.TemporaryDirectory(prefix="g2-case-leaves-target-") as tmp:
            output = Path(tmp) / "leaves.o"
            subprocess.run([
