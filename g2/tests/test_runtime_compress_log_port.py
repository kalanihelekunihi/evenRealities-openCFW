...
class CompressLogPortRuntimeTests(unittest.TestCase):
    def test_host_behavior(self):
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory) / "compress_log_port_host"
            subprocess.run(
                [
                    "clang", "-std=c11", "-Wall", "-Wextra", "-Werror",
                    str(ROOT / "tests/fixtures/compress_log_port_host.c"),
                    "-o", str(output),
                ],
                check=True,
                cwd=ROOT,
    ...
    def test_freestanding_arm_compile(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory) / "compress_log_port.o"
            subprocess.run(
                [
