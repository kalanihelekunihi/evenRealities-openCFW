...
SOURCE = ROOT / "components/apollo_main/core_overlay/ota_transport.c"
FIXTURE = ROOT / "tests/fixtures/ota_transport_host.c"
SELECTORS = {
    "RECEIVE": "OTA_ReceivePacket",
    "SEND": "OTA_SendPacket",
    "STATE": "OTA_GetTransferState",
}


class OtaTransportCandidateTests(unittest.TestCase):
    def test_host_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            executable = Path(directory) / "ota-transport"
            subprocess.run([
        ...
        with tempfile.TemporaryDirectory() as directory:
            for selector, expected in SELECTORS.items():
                ...
                subprocess.run([
                    ...
                    "-DOPEN_CFW_OTA_" + selector + "_ONLY=1", "-c", str(SOURCE),
                    "-o", str(obj),
                ], cwd=ROOT, check=True)
