...
import analyze_g2_nanopb_field_default as audit  # noqa: E402


class NanopbFieldDefaultAuditTests(unittest.TestCase):
    def test_authenticated_boundary_and_dependency_closure(self) -> None:
        report = audit.analyze()
        self.assertEqual(report["stock"]["size"], 272)
        self.assertEqual(report["stock"]["sha256"], audit.STOCK_SHA256)
        self.assertEqual(len(report["ingress"]["direct"]["bl"]), 1)
        self.assertFalse(report["ingress"]["interior"])
        self.assertFalse(report["ingress"]["stored_patterns"])
        self.assertEqual(len(report["outgoing"]), 5)
        self.assertEqual(
        ...
        self.assertEqual(report["decision"]["production_fixed_stock_call_sites"], 0)
        self.assertEqual(report["decision"]["fixed_stock_helper_families"], 3)
        self.assertEqual(report["decision"]["eliminated_memory_fill_call_sites"], 1)
