...
class State(ctypes.Structure):
    _fields_ = [
        ...
        ("force_buck_active", ctypes.c_uint8),
    ]


def expected_force(state: State) -> int:
    if (
    ...
    for index, control in enumerate(state.timer_ctrl):
        ...
        clock_matches = (
            clock < 6 or 19 <= clock < 25 or 0x100 <= clock < 0x1E0
        )
        if (
            control & 1
...
class BootloaderSpotmgrBuckDeepsleepState42a08cTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temporary = tempfile.TemporaryDirectory()
        library = Path(cls.temporary.name) / "spotmgr_buck_deepsleep.so"
    ...
    def test_source_is_reviewable_bsd_c_without_raw_opcodes(self):
        ...
        self.assertIn("clock < 6U", body)
        self.assertIn("clock < 0x1e0U", body)
        self.assertNotIn(".byte", body)
        self.assertNotIn(".word", body)
        self.assertNotIn(".inst", body)

    def test_portable_classifier_over_randomized_hardware_snapshots(self):
        rng = random.Random(0x42A08C)
        for _ in range(100_000):
            ...
            state.force_buck_active = rng.randrange(256)
            expected = expected_force(state)
            self.function(ctypes.byref(state))
            self.assertEqual(state.force_buck_active, expected)

    def test_dual_toolchain_body_is_exact_after_stimer_relocation(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            for profile, compiler in PROFILES.items():
                ...
                self.assertEqual(report["relocation_count"], 1, profile)
