...
class Tests(unittest.TestCase):
 ...
 def test_audit(s):
    r=analyzer.audit();s.assertTrue(r["production"]["routed"]);s.assertEqual(r["production"]["boundary_status"],"source_compiled");s.assertEqual(r["production"]["compiled_bytes"],672);s.assertEqual(r["production"]["source_owned_bytes"]+r["production"]["retained_official_bytes"],146994);s.assertEqual(r["production"]["next_frontier"],0x424E84);s.assertEqual(r["next_code_frontier"],{"start":0x425066,"end":0x4250F0,"identity":"am_hal_mspi_enable","bytes":138,"source_compiled_bytes":128,"retained_unreachable_tail_bytes":10,"status":"source-compiled-with-retained-unreachable-tail"});s.assertEqual(r["hardware_validation"],"blocked by unavailable physical evidence");s.assertEqual(r["hardware_operations"],[])
