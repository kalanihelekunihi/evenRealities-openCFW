...
ROOT=Path(__file__).resolve().parents[1];SOURCE=ROOT/"components/bootloader/core_overlay/runtime_mspi_configure_424af0.c";FIXTURE=ROOT/"tests/fixtures/bootloader_runtime_mspi_configure_host.c";sys.path.insert(0,str(ROOT/"tools"));import analyze_g2_bootloader_mspi_configure_424af0 as analyzer
class Tests(unittest.TestCase):
 @classmethod
 def setUpClass(c):
  c.t=tempfile.TemporaryDirectory();o=Path(c.t.name)/("x.dylib" if sys.platform=="darwin" else "x.so");cmd=["/usr/bin/clang","-std=c11","-O2","-Wall","-Wextra","-Werror",str(FIXTURE)]+(["-dynamiclib"] if sys.platform=="darwin" else ["-shared","-fPIC"]);subprocess.run([*cmd,"-o",str(o)],check=True,capture_output=True,text=True);c.x=ctypes.CDLL(str(o));c.x.open_cfw_test_mspi_configure_reset.argtypes=[ctypes.c_uint32]*8;c.x.open_cfw_test_mspi_configure_run.argtypes=[ctypes.c_uint32];c.x.open_cfw_test_mspi_configure_run.restype=ctypes.c_uint32;c.x.open_cfw_test_mspi_configure_state.argtypes=[ctypes.c_uint32]*3;c.x.open_cfw_test_mspi_configure_state.restype=ctypes.c_uint32;c.x.open_cfw_test_mspi_configure_register.argtypes=[ctypes.c_uint32]*2;c.x.open_cfw_test_mspi_configure_register.restype=ctypes.c_uint32
 @classmethod
 ...
 def test_audit(s):
  r=analyzer.audit();s.assertTrue(r["production"]["routed"]);s.assertEqual(r["production"]["boundary_status"],"source_compiled");s.assertEqual(r["production"]["compiled_bytes"],152);s.assertEqual(r["production"]["source_owned_bytes"]+r["production"]["retained_official_bytes"],146994);s.assertEqual(r["production"]["next_frontier"],0x424B88);s.assertEqual(r["next_code_frontier"],{"start":0x424BE4,"end":0x425066,"identity":"am_hal_mspi_device_configure","bytes":1154,"source_compiled_bytes":672,"retained_unreachable_tail_bytes":482,"status":"source-compiled-with-retained-unreachable-tail"});s.assertEqual(r["hardware_validation"],"blocked by unavailable physical evidence");s.assertEqual(r["hardware_operations"],[])
