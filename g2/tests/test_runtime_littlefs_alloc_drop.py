...
SOURCE = (
    ...
    / "apollo_main"
    / "core_overlay"
    / "runtime_littlefs_alloc_drop.c"
)
FIXTURE = (
    ROOT
    / "tests"
    / "fixtures"
    / "runtime_littlefs_alloc_drop_host.c"
)
UPSTREAM_FIXTURE = (
    ROOT
    / "tests"
    / "fixtures"
    / "runtime_littlefs_alloc_drop_upstream_oracle_host.c"
)
...
MAIN_START = 0x004CB0E6
MAIN_END = 0x004CB0F6
MAIN_CKPOINT = 0x004CB0E0
MAIN_CALLERS = (
    (0x004CB17A, "fff7b4ff"),
    (0x004CF266, "fbf73eff"),
)
BOOT_BASE = 0x00410000
BOOT_START = 0x00410DEE
BOOT_END = 0x00410DFE
BOOT_CKPOINT = 0x00410DE8
BOOT_CALLERS = (
    (0x00410E82, "fff7b4ff"),
    (0x0041493E, "fcf756fa"),
)
...
STOCK_BYTES = "80b5002181650021c165fff7f6ff01bd"
TARGET_SHA256 = (
    "e5e78109621631cb174d82b06ba2542dfa669e1919c3d80982ab059844e5c4f8"
)
TARGET_BYTES = "0021c1658165c16e01667047"
SOURCE_SHA256 = (
...
class RuntimeLittlefsAllocDropTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.candidate = cls.compile_host_library(
            FIXTURE,
            temporary / cls.library_name("runtime_littlefs_alloc_drop"),
        )
        cls.oracle = cls.compile_host_library(
            UPSTREAM_FIXTURE,
            temporary / cls.library_name(
                "runtime_littlefs_alloc_drop_oracle"
            ),
        )
        cls.candidate_api = cls.bind_api(
            cls.candidate,
            "open_cfw_test_littlefs_alloc_drop_",
        )
        cls.oracle_api = cls.bind_api(
            cls.oracle,
            "open_cfw_oracle_littlefs_alloc_drop_",
        )

        cls.target_object = temporary / "runtime_littlefs_alloc_drop.o"
        subprocess.run(
            [
    ...
    def reset_both(
        ...
        args = (pattern, size, next_, checkpoint, block_count)
        self.candidate_api["reset"](*args)
        self.oracle_api["reset"](*args)
    ...
    def test_recovered_target_lfs_layout_is_exact(self) -> None:
        ...
        self.assertEqual(
            Littlefs32.lookahead.offset + LittlefsLookahead32.next.offset,
            0x5C,
        )
        self.assertEqual(
    ...
    def test_value_classes_match_pristine_upstream(self) -> None:
        cases = (
            (0x00, 0, 0, 0, 0),
            (0x5A, 1, 2, 3, 4),
            (0xFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF),
            (0xA5, 0x7FFFFFFF, 0x80000000, 0xDEADBEEF, 0x00000BC0),
            (0x3C, 0x80000000, 0x7FFFFFFF, 0x00000BC0, 0xDEADBEEF),
        )
        for case in cases:
            with self.subTest(case=case):
                self.reset_both(*case)
                self.assertEqual(
                ...
                oracle_outside = self.oracle_api["outside_checksum"]()
                ...
                for api in (self.candidate_api, self.oracle_api):
                    self.assertEqual(api["size"](), 0)
                    self.assertEqual(api["next"](), 0)
                    self.assertEqual(api["checkpoint"](), case[4])
                    self.assertEqual(api["block_count"](), case[4])
                    self.assertEqual(api["outside_checksum"](), candidate_outside)
                self.assertEqual(
                    self.candidate_api["checksum"](),
                    self.oracle_api["checksum"](),
                )
    ...
    @_APPLE_ONLY
    def test_target_object_is_closed_and_dependency_free(self) -> None:
        function = self.symbols["open_cfw_littlefs_alloc_drop"]
    ...
    @_APPLE_ONLY
    def test_target_opcodes_encode_all_four_authenticated_offsets(
        self,
    ) -> None:
        zero, store_next, store_size, load_count, store_checkpoint, return_ = (
            struct.unpack("<6H", self.target_text)
        )
        self.assertEqual(zero, 0x2100)
        for instruction, expected_offset in (
            (store_next, 0x5C),
            (store_size, 0x58),
            (store_checkpoint, 0x60),
        ):
            self.assertEqual(instruction & 0xF800, 0x6000)
            self.assertEqual((instruction >> 6 & 0x1F) * 4, expected_offset)
            self.assertEqual(instruction >> 3 & 0x07, 0)
            self.assertEqual(instruction & 0x07, 1)
        self.assertEqual(load_count & 0xF800, 0x6800)
        self.assertEqual((load_count >> 6 & 0x1F) * 4, 0x6C)
        self.assertEqual(load_count >> 3 & 0x07, 0)
        self.assertEqual(load_count & 0x07, 1)
    ...
    def test_source_and_pristine_oracle_are_authenticated(self) -> None:
        ...
        for token in (
            ...
            "Apollo-main [0x004CB0E6, 0x004CB0F6)",
            "[0x00410DEE, 0x00410DFE)",
            "size\n        ) == 0x58U",
            "next\n        ) == 0x5CU",
            "checkpoint\n        ) == 0x60U",
            "block_count\n    ) == 0x6CU",
        ...
        self.assertIn("../../third_party/littlefs/lfs_util.c", oracle_source)
        self.assertIn("lfs_alloc_drop(", oracle_source)
        self.assertEqual(
    ...
    def test_dual_image_stock_body_and_outgoing_call_are_exact(self) -> None:
        import apollo_overlay

        for image, base, start, end, checkpoint in (
            (self.main, MAIN_BASE, MAIN_START, MAIN_END, MAIN_CKPOINT),
            (self.boot, BOOT_BASE, BOOT_START, BOOT_END, BOOT_CKPOINT),
        ):
            ...
            self.assertEqual(
                self.span(image, base, checkpoint, checkpoint + 6).hex(),
                "c16e01667047",
            )
    ...
    def test_official_images_and_configuration_are_exact(self) -> None:
        ...
        self.assertEqual(hashlib.sha256(self.boot).hexdigest(), BOOT_SHA256)

        configurations = (
            (self.main, MAIN_BASE, 0x006E83A4),
            (self.boot, BOOT_BASE, 0x00431070),
        )
        for image, base, address in configurations:
            words = struct.unpack(
                "<21I",
                self.span(image, base, address, address + 84),
            )
    ...
    def test_dual_image_caller_and_reference_topology_is_exact(self) -> None:
        cases = (
            (
                ...
                MAIN_END,
                MAIN_CALLERS,
                "1bd8116bcd28734549add50348fa07c27"
                "f848a5fc5fb3f83d9805c92459ea819",
                "349ba2c5ad9de3a017b91cd07056f91db"
                "cf6eaf968806a5577f95f08be65a2d0",
            ),
            (
                self.boot,
                BOOT_BASE,
                ...
                BOOT_END,
                BOOT_CALLERS,
                "a7b1297b66cff8986f860dc5df9c7d561"
                "2df6e46a930dfd9b1f477efbacffcef",
                "db918078f3ebe1a093a99485e95ddc67da"
                "982e5703e77e24248b35d2e5e2c9b4",
            ),
        )
        for image, base, start, end, callers, address_hash, call_hash in cases:
    ...
    def scan_topology(
        ...
        for offset in range(0, len(image) - 1, 2):
            ...
            for target in (
                RuntimeLittlefsAllocDropTests.narrow_branch_targets(
                    address,
                    halfword,
                )
            ):
