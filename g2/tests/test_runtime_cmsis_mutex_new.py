...
class RuntimeCmsisMutexNewTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        builder_spec = importlib.util.spec_from_file_location(
            "open_cfw_canonical_core_builder_for_cmsis_mutex",
            CURRENT_CONFIG.parent / "build_component.py",
        )
        if builder_spec is None or builder_spec.loader is None:
            raise AssertionError("cannot load canonical Apollo core builder")
        canonical_builder = importlib.util.module_from_spec(builder_spec)
        builder_spec.loader.exec_module(canonical_builder)
        cls.current_report = canonical_builder.build(
            root=ROOT,
    ...
    def test_production_config_report_redirect_and_artifacts_are_exact(
        ...
        self.assertEqual(component_report["size"], 3_956_672)
        self.assertEqual(
        ...
        self.assertEqual(len(package), 4_750_780)
        self.assertEqual(sha256(package), PRODUCTION_PACKAGE_SHA256)
        # Later LC3/service/CFF composition extends the final component after
        # the core overlay, so ``len(component) - len(overlay)`` no longer
        # identifies the overlay payload start.  Map the admitted runtime
        # address through the component preamble instead.
        component_offset = (
        ...
        self.assertEqual(
            ...
            component[component_offset:component_offset + 116],
        )
