...
class EfsTransportAuditTest(unittest.TestCase):
 ...
 def test_ownership(self):self.assertEqual(self.r['identity']['embedded_third_party_definitions'],[]);self.assertTrue(self.r['production']['production_routed']);self.assertFalse(self.r['production']['software_functional_gap']);self.assertEqual(self.r['production']['hardware_validation'],'deferred by project direction')
