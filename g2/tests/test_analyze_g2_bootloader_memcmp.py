...
class AnalyzeBootloaderMemcmpTests(unittest.TestCase):
    def test_fail_closed_audit(self) -> None:
        completed = subprocess.run(["python3", "tools/analyze_g2_bootloader_memcmp.py", "--json"], cwd=ROOT, check=True, capture_output=True, text=True)
        ...
        self.assertEqual(report["source"]["size"], 28)
        self.assertEqual(
