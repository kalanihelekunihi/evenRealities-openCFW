...
class ThreadRingTests(unittest.TestCase):
 ...
 def test_no_embedded_dependency_or_production_route(self):
  self.assertEqual(self.r['identity']['embedded_third_party_definitions'],[]);self.assertFalse(self.r['production']['production_routed']);self.assertTrue(all(self.r['behavior'].values()))
