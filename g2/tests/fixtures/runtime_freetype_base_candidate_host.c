 ...
 *
 * Host-only lifecycle and allocation fixture for the G2 FreeType candidate.
 * See third_party/freetype/LICENSE.
 */

#include "runtime_freetype_base_candidate.h"
#include "runtime_freetype_base_cluster_candidate.h"
...
static struct open_cfw_freetype_base_candidate test_candidate;
...
{
    test_candidate = (struct open_cfw_freetype_base_candidate){0};
...
int open_cfw_test_freetype_init(void)
{
    ...
    return open_cfw_freetype_base_init(&test_candidate, &ports);
}

int open_cfw_test_freetype_done(void)
{
    return open_cfw_freetype_base_done(&test_candidate);
}

FT_Library open_cfw_test_freetype_library(void)
{
    return open_cfw_freetype_base_library(&test_candidate);
}
