...
class Tests(unittest.TestCase):
 ...
 def test_audit(s):
  r=analyzer.audit();s.assertTrue(r["production"]["routed"]);s.assertEqual(r["production"]["boundary_status"],"source_compiled_with_authenticated_unreachable_tails");s.assertEqual(r["production"]["source_owned_tranche_bytes"],404);s.assertEqual(r["production"]["retained_unreachable_tail_bytes"],582);s.assertEqual(r["production"]["next_frontier"],0x426506);s.assertEqual(r["production"]["next_frontier_status"],"already-source-routed-interrupt-clear");s.assertTrue(r["bounded_mspi_code_closed"]);s.assertTrue(r["candidate_semantics_closed"]);s.assertEqual(r["adjacent_control"]["status"],"production-source-with-authenticated-unreachable-tail");s.assertEqual(set(r["profiles"]),{"apple-clang","linux-clang"});s.assertEqual(r["hardware_validation"],"blocked by unavailable physical evidence");s.assertEqual(r["hardware_operations"],[])
