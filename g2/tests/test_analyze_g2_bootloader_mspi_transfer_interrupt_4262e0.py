...
class Tests(unittest.TestCase):
 ...
 def test_audit(s):r=analyzer.audit();s.assertFalse(r["production"]["routed"]);s.assertEqual(r["production"]["boundary_status"],"official_blob");s.assertEqual(r["production"]["source_owned_bytes"]+r["production"]["retained_official_bytes"],147296);s.assertEqual(r["production"]["next_frontier"],0x426506);s.assertFalse(r["bounded_mspi_code_closed"]);s.assertTrue(r["candidate_semantics_closed"]);s.assertEqual(r["adjacent_control"]["status"],"official_blob");s.assertEqual(r["hardware_validation"],"deferred by project direction");s.assertEqual(r["hardware_operations"],[])
