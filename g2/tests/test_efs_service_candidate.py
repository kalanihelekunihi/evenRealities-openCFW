...
FIXTURE = ROOT / "tests/fixtures/efs_service_host.c"
SELECTORS = {
    "REPLY": "_evenEfsReplyToAPP",
    "CRC": "_fileCaculateCRC",
    "COMMAND": "_efsFileCmdParse",
    "RAW": "_efsFileRawDataParse",
    "EXPORT": "_efsExportFileParse",
    "DISPATCH": "EFS_FrameDispatch",
    "STATUS4": "EFS_NotifyStatus4",
    "STATUS2": "EFS_NotifyStatus2",
    "STATUS5": "EFS_NotifyStatus5",
    "ACTIVE": "EFS_TransferActive",
    "INIT": "EFS_ServiceInit",
    "CANCEL": "EFS_CancelExport",
}


class EfsServiceCandidateTests(unittest.TestCase):
    def test_host_behavior(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            executable = Path(directory) / "efs-service"
            subprocess.run([
        ...
        with tempfile.TemporaryDirectory() as directory:
            for selector, expected in SELECTORS.items():
                ...
                subprocess.run([
                    ...
                    "-DOPEN_CFW_EFS_SERVICE_" + selector + "_ONLY=1", "-c",
    ...
    def test_source_is_nonempty_and_pinned_by_digest_shape(self) -> None:
        ...
        self.assertGreater(len(data), 28_000)
