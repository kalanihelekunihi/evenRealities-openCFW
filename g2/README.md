...
The community-facing artifact is the deterministic, official-payload-free
...
The current checked assessment authenticates 157 inputs. The canonical Apple
profile accounts for 424,703 production-source, 426,474 generated, 30,636
candidate-not-routed, 3,795,983 retained/external, and zero unclassified bytes
across a 4,677,796-byte payload; the outer package is 4,678,740 bytes. Of those
bytes, 3,826,619 remain release-blocking. These are classification results, not
a source-completion or public-binary authorization claim:
`source_complete=false`, `release_authorized=false`, all six binary
redistribution authorities remain unresolved, hardware validation is blocked
by unavailable physical evidence, and the recorded hardware operation list is
empty.

The current dual-profile receipt (schema 3) pins these Apollo-main build
identities:

| Profile | Overlay size / SHA-256 | Component size / SHA-256 | Package size / SHA-256 |
| --- | --- | --- | --- |
| Apple Clang | 362,272 / `8c80c3fa53a89c77d145533f59f63389dfa31f968642f783323ed81ac81be5ae` | 3,885,668 / `898d5efb1430dc0c3e0b8b7e26823a653952114ffeab0d3ae6e89d8925301ef5` | 4,678,740 / `d569793138c6bc2ee456536daee59dcef0bb6051034ed966f7144083790a777a` |
| Linux Clang | 154,604 / `4caa6c35e2c8f559d7668511d8c36fd19ba95a94a8762215f9bed4ba91e006c6` | 3,678,000 / `45d32718d333b61718d7ebeededf4692760608af01951b98501189f5e809eccd` | 4,471,056 / `f7260f93e2c87f2403e14f5a8e6ae1436233cce2990c386a1cde48d2e8133e31` |

Touch currently contributes 512 generated bytes, 14,510 unrouted candidate
bytes, 19,442 retained/external bytes, and zero unclassified bytes. Its new
[`g2-touch-final-source-candidate-provenance.tsv`](tools/manifests/g2-touch-final-source-candidate-provenance.tsv)
records six mixed-license semantic-only provenance rows; all explicitly deny
production ELF ownership. The verified community deliverable remains a
source-only archive for this hybrid source-overlay workflow. It distributes no
official or retained firmware bytes; recipients hydrate the six providers from
their own authenticated official package for local software-only builds.

EM9305's persisted
[final readiness ledger](tools/manifests/em9305-final-source-readiness.tsv)
accounts for all 175 residual spans and all 33,658 residual bytes: 23 spans /
1,240 bytes have concrete source available but are not production-routed, 25 /
8,348 are typed unsupported
external boundaries, and 127 / 24,070 are unavailable proprietary controller
code. No residual span or byte is unclassified. This is accounting closure,
not controller source completion: the component remains stock-retained,
`source_complete=false`, and publicly binary-release-blocking.
...
Requirements are Python 3.9 or newer, GNU `make`, a shell, and the reviewed
...
constructor-side lifecycle seams. Separate community-source admissions now
cover the TrueType driver boundary and the CFF policy surface; the latter pins
47 mapped functions / 12,062 stock bytes and exposes Adobe-only hinting plus
validated stem-darkening controls. Neither admission is routed into the stock
overlay. FreeType image replacement remains blocked on the remaining
configuration toggles, exact compiler/linker details and destructor symbols,
external font payload recovery, placement evidence, and authorized rendering
validation. The
...
The source-divergent artifact described at this historical milestone passed
structural checks and offline flasher inspection. The then-available log was
incomplete and its inference that no physical installation had succeeded is
superseded: multiple firmware flashing attempts using the
`evenRealities-webflasher` codebase have succeeded. Hardware qualification of
the current image is blocked by unavailable physical evidence and requires future
authorized device evidence. The
...
and it ended at `0x00434516`. At that milestone, the bootloader lacked separate
hardware-qualification evidence and was materially riskier to program than the
application; current qualification remains blocked by unavailable physical evidence.
...
the remaining 33,658-byte code-or-mixed set is now fully typed by the final
readiness ledger: 1,240 bytes have concrete but unrouted source, 8,348 bytes
are unsupported external boundaries, and 24,070 bytes are unavailable
proprietary controller code. No byte in that set is unclassified, but all
remain cut-forward and the controller is not source-complete. The archive
increment includes 62 exact ISO/BIG controller bodies, so the non-ISO profile
header is not a complete final-link configuration claim. Packetcraft's public r20.05c
...
bytes with SHA-256
`03292baa960e39beb368b32a0b93f3f68d13caf6db121a2bb6020363c366afa0`.
No hardware was accessed or flashed for this software-only closure; the
remaining platform scheduler-start and STIMER/tickless rows are separate.
Physical timing validation is blocked by unavailable physical evidence; future
qualification requires authorized timing evidence.
...
No hardware was accessed or flashed for this software-only closure. Live CM55
preemption, exception return, overflow-hook, trace-concurrency, and STIMER
timing validation is blocked by unavailable physical evidence; future qualification
requires authorized G2 probe/capture evidence.
...
No hardware was accessed or flashed for this software-only closure. Actual
watchdog reset timing and reset-cause validation is deferred by project
direction; future qualification requires authorized G2 capture evidence.
...
No hardware was accessed or flashed for this software-only closure. Physical
mutex scheduling, role/display gating, record delivery, and visible health
behavior qualification is blocked by unavailable physical evidence and requires future
authorized G2 evidence.
...
No hardware was accessed or flashed for this software-only closure.
Pairing/controller timing, pool pressure, disconnect races, and peer
interoperability qualification is blocked by unavailable physical evidence and requires
future authorized G2/EM9305 physical evidence.
...
interoperability qualification is blocked by unavailable physical evidence and requires
future authorized G2/EM9305 evidence; the remaining security role modules and
overall firmware ledger are not complete.
...
behavior, and peer interoperability qualification is deferred by project
direction and requires future authorized G2/EM9305 evidence. The wider SMP,
...
No hardware was accessed or flashed for this software-only closure. Real
scheduler timing, controller races, and peer interoperability qualification is
blocked by unavailable physical evidence and requires future authorized G2/EM9305
evidence; the remaining SMP state/action units, app policy, cryptographic
backend, and overall firmware ledger remain open.
...
No hardware was accessed or flashed for this software-only closure. Physical
controller timing, disconnect races, legacy/LESC peer interoperability, and
stale-queue behavior qualification is blocked by unavailable physical evidence and
requires future authorized G2/EM9305 evidence; the remaining SMP action/state
units and overall firmware ledger remain open.
...
validation is blocked by unavailable physical evidence and requires future authorized
G2/EM9305 evidence; remaining SMP action/state units and the firmware remain
incomplete.
...
and peer-interoperability qualification is blocked by unavailable physical evidence and
requires future authorized G2/EM9305 hardware evidence. Shared Secure
Connections actions are source-routed;
