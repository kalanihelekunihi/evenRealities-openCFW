#!/usr/bin/env python3
"""Register the G2 SPOT-manager power-state transition-sequence selector."""
...
EVIDENCE = (
    "docs/research/"
    "g2-bootloader-spotmgr-state-transition-sequence-42a2b4-source-closure.md"
)
BOOT_BASE = 0x00410000
FUNCTION = "open_cfw_bootloader_spotmgr_state_transition_sequence_42a2b4"
START = 0x0042A2B4
END = 0x0042A43A
STOCK_SHA = "c02ca4144181ebe16c3dffc47e1bec89a89fbb832fa8bb134b38dd8bf287444f"
UNRELOCATED_SHA = "e0fad5fe49ce4fde2b8a7371bc7a03824d8a273e9003c735317b3bb7075a7cf7"
SOURCE_SHA = "4741430855fd4a1344b48d5e5e7e6fc7b5ab1a6e2d389c52808d7c04492bf014"
...
def source_record() -> dict[str, Any]:
    ...
    if len(payload) != 11_291 or digest(payload) != SOURCE_SHA:
        raise SystemExit("SPOT-manager transition-sequence source identity changed")
    return {
        "path": SOURCE.relative_to(ROOT).as_posix(),
        "size": len(payload),
        "sha256": digest(payload),
        "license": "BSD-3-Clause",
        "origin": "Apollo510-compatible SPOT-manager transition-sequence selector",
...
def update_census() -> None:
    ...
    for row in rows:
        ...
        if int(row["end"], 16) != END or row["sha256"] != STOCK_SHA:
            raise SystemExit("SPOT-manager transition-sequence census identity changed")
        row.update({
            "kind": "source_function",
            "name": "spotmgr_state_transition_sequence_42a2b4",
            "disposition": "source_owned_production",
            "provider": "AmbiqSuite Apollo510 SPOT-manager transition selector",
            "license_status": "BSD-3-Clause",
            "evidence": (
                "reviewed production C with exact dual-toolchain bytes, one "
                "authenticated aligned-memcpy provider edge, and exhaustive "
                "valid-state-pair host tests"
            ),
        })
        seen = True
    if not seen:
        raise SystemExit("SPOT-manager transition-sequence census row not found")
...
def main() -> int:
    ...
    if len(stock) != END - START or digest(stock) != STOCK_SHA:
        raise SystemExit("authenticated SPOT-manager transition selector changed")
    ...
    update_census()
    print("registered SPOT-manager transition-sequence selector at 0x0042A2B4")
    return 0
