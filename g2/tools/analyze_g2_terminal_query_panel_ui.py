#!/usr/bin/env python3
"""Fail-closed zero-anchor linked-object audit for app\\gui\\terminal\\terminal_query_panel_ui.c."""
import csv
import hashlib
import json
import struct
...
FM = ROOT / "tools/manifests/g2-terminal-query-panel-ui-function-map.tsv"
CL = ROOT / "tools/manifests/g2-terminal-query-panel-ui-closure.tsv"
PINS = {FM: "871b04348f40f6f005827a13eb7684e2538f78c13fa60430432fc0252677776d", CL: "1369bc2d6485d331c33565e6954bdcfc327f5749bb1e7ce548081576940f3e1a"}
RETAINED = 'app\\gui\\terminal\\terminal_query_panel_ui.c'
FULL_PATH = 'D:\\01_workspace\\s200_ap510b_iar_git\\app\\gui\\terminal\\terminal_query_panel_ui.c'
PATH_RUN = 0x6f0448
CELLS = (6208576,)
CELL_REFS = {6208576: (6207498, 6207648)}
ALL_REFS = (6207498, 6207648)
F = ((6206520, 6206586), (6206586, 6206710), (6206710, 6206838), (6206838, 6207006), (6207006, 6207046), (6207046, 6207706))
PHYS = (6206520, 6207706)
FOREIGN = ()
ESCAPES = ()
INDIRECT = ()
BL_ENTRY = ((6180188, 6206838), (6180202, 6206586), (6180290, 6206838), (6186086, 6207006), (6191542, 6206710), (6191680, 6207046), (6191726, 6207046), (6206658, 6206520), (6206768, 6206520), (6206884, 6206520), (6207040, 6206838))
BL_STRICT = ()
BW_ENTRY = ()
B16_ENTRY = ()
STORED_RAW = ()
GUARD_BEFORE = "7e2d7d9e3c7cf16864564efb6110fe72e1e9ba90b3199c00ea8ecc0ba3e72210"
GUARD_AFTER = "febe1edea31d59779b75a25707cd70c9b9b9386d2a4e03657df492bc7300bd0c"
TAGS = ((7149448, '[terminal.ui]query align before clamp: idx=%u keep2=%d panel_h=%d view_h=%d pad_t=%d pad_b=%d gap_b=%d safe=%d options_top=%d sel_bottom=%d anchor_bottom=%d target_y=%d max_y=%d browsing=%d'), (7531436, '[terminal.ui]query align target: idx=%u target_y=%d'))
EXPECTED = {'body_bytes': 1186, 'body_concat_sha256': '2b042f6fdd4cbf149521f3ee7c0e51e550434faa029b665c817dc46374e9927f', 'reachable_instructions': 435, 'reachable_instruction_digest': '092050ebf061a12787ef2f211c010fd028d2ec4dba39d97d5da15bab10f9f7b0', 'direct_body_calls': 48, 'direct_body_call_digest': 'be336e40a2f353e3bf347554e6cd668e0954b343045ced66913c30da6704d22f', 'internal_direct_body_calls': 4, 'outer_pool_bytes': 0, 'outer_pool_sha256': 'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855', 'physical_bytes': 1186, 'physical_sha256': '2b042f6fdd4cbf149521f3ee7c0e51e550434faa029b665c817dc46374e9927f', 'path_literal_references': 2}
...
def analyze(image=IMAGE):
    ...
    if any(x.get("path", "").replace("\\", "/").split("/")[-1].lower() == 'terminal_query_panel_ui.c' for x in overlay["sources"]):
