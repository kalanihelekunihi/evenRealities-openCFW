...
PINS={FM:'c2c4a76be4479a4439cee52987bb339a62ba838eed14af27a47961cdf300edbb',PM:'3b725828f8a539fb37aa2d05d835abade1dca4ebbb54ee4b537a67dfb3574827',CL:'a8ccf601bf1da89d16e77130c14c6bf623d3902ae215e054ed041052ff71debe'}
...
SOURCE_PIN=(28464,'3b64f8a23402d993532b8affcd4c21eda620f33c02bafcbe06bb392864e0a9d0')
...
def analyze(image=IMAGE):
 ...
 if len(leaves)!=31 or len(names)!=31 or not names<=set(overlay['functions']) or sum(x['expected']['size'] for x in leaves)!=1602 or sum(x['expected']['offset']-(leaves[i-1]['expected']['offset']+leaves[i-1]['expected']['size']) if i else x['expected']['offset']-254048 for i,x in enumerate(leaves))!=36 or sum(len(x['relocations']) for x in leaves)!=106:raise c.AuditError('sensor-hub production leaf closure changed')
 if any(x.get('profiles')!=['apple-clang'] or not x.get('strict_relocation_contract') or x.get('source',{}).get('license')!='MIT' for x in leaves):raise c.AuditError('sensor-hub production policy changed')
