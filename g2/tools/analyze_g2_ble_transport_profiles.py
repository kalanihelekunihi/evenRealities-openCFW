...
PINS={FUNCTION_MAP:'a31155ed0cbe4e73267115dc78a7f8fcc30081f44bf601608b0ace2646a9f989',CLOSURE:'741235879030a6f38b6b6d004bf0278af128887289e7c5dc4923b1cfe1d92ed7',PROVENANCE:'a53a7958bec483e637f0598f82fae63c2165bde4fad355b85fc280d788de89e9'}
SOURCE_PIN=(17161,'d61cbcf6a26265e5dd835f4ec9f008cc1c19ec0fe5fd33f1810b9b1e39951c58')
...
LEAF_DIGEST='9ac2891014938c81f010f65dd781f8ca8baaf2dfdb8de6253c417406adac97de';PATCH_DIGEST='c58513379fe11707c31e2b28ada7800d770a8116ea0487204accbbf059ac78a4';BUILT_DIGEST='02114a792d30ee5c83d3978ba5cc779b8564113ef9659e36a20d23120c5d8da8';REGION_DIGEST='e3419dc0405c5f93f7c1c9ebdfcba6e38eafcfa6a27a19c4e601fca8ecce0624'
MODULES={
...
def analyze(image:Path=IMAGE)->dict[str,Any]:
 ...
 for name,x in MODULES.items():
  ...
  if metrics.get('production_routed')!='true' or metrics.get('software_functional_gap')!='false' or metrics.get('hardware_validation')!='blocked by unavailable physical evidence' or any(int(metrics[k])!=v for k,v in production_checks.items()):raise AuditError(f'{name} production closure metric changed')
 ...
 if any(x.get('profiles')!=['apple-clang'] or not x.get('strict_relocation_contract') or x.get('source',{}).get('license')!='MIT' for x in leaves):raise AuditError('production BLE transport-profile leaf policy changed')
 ...
 previous=222948;alignment=0
 for leaf in leaves:alignment+=leaf['expected']['offset']-previous;previous=leaf['expected']['offset']+leaf['expected']['size']
 if alignment!=10 or previous!=224198:raise AuditError('production BLE transport-profile placement changed')
 ...
 validate_apollo_main_artifacts(ROOT,AuditError,'BLE transport profiles')
