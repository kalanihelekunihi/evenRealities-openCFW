...
PINS={FUNCTION_MAP:'a31155ed0cbe4e73267115dc78a7f8fcc30081f44bf601608b0ace2646a9f989',CLOSURE:'20b1dda7b52f3c7f063530bcbff6bb9371a0dbb7609dc6e5c62d0d28852424be',PROVENANCE:'a53a7958bec483e637f0598f82fae63c2165bde4fad355b85fc280d788de89e9'}
...
def analyze(image:Path=IMAGE)->dict[str,Any]:
 ...
 for name,x in MODULES.items():
  ...
  if metrics.get('production_routed')!='true' or metrics.get('software_functional_gap')!='false' or metrics.get('hardware_validation')!='deferred by project direction' or any(int(metrics[k])!=v for k,v in production_checks.items()):raise AuditError(f'{name} production closure metric changed')
