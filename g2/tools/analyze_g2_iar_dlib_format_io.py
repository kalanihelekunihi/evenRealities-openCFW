...
def analyze(
    ...
    for unit in EXPECTED_UNITS:
        ...
        bl = [site for site, kind, tgt in branches if kind == "bl" and tgt == start]
        bw = [site for site, kind, tgt in branches if kind == "bw" and tgt == start]
...
def map_tsv(report: dict[str, Any]) -> str:
    key_by_address = {
        address: key for key, _text, address, _digest in SIGNATURE_STRINGS
    }
    lines = [
