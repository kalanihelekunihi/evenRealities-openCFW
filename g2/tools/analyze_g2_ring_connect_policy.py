...
PINS={FM:'d368e986ca1a14b17e9c86d731d3711c9f70b5c5634483699294696cac6a64e9',PM:'2ffd806820e42a16158ccc6c1638dfeedd8793cd7cb9d4e709a6fd5138e36a31',CL:'3175bdb2f1424fd5708a47cab0535c39fb7b1c3f7c8b5c13f6b270817216e6eb'}
SOURCE_SHA='ec34969d4313cab8b91581d33c210047930859edfd03c933a27f6f341515a0ee'
...
SOURCE_OFFSETS=(251580,251584,251608,251632,251684,251716,251760,251832,251888,251920,251940,251992,252036,252100,252148)
...
def analyze(image=IMAGE):
 ...
 if len(source)!=10641 or sh(source)!=SOURCE_SHA:raise c.AuditError('production Ring policy source changed')
 ...
 if sum(len(x['relocations']) for x in leaves)!=24 or any(x.get('profiles')!=['apple-clang'] or x.get('strict_relocation_contract') is not True or x.get('source',{}).get('sha256')!=SOURCE_SHA or x.get('source',{}).get('license')!='MIT' for x in leaves):raise c.AuditError('production Ring policy source authentication changed')
