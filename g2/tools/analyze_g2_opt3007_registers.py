...
PINS={FM:"31b30dc9653fa501c37ffe5052098aaf8efc4d070f7279daa0d1b7a00e188047",CL:"03e4fab29bfd1186ac1e31133bebbf9e52fd0ef29a6943cac2cacaea07a9d715",PM:"8a784bba12262ffd2033c2bff3583a4d0f0dc61635450bca39f66c0c80c957a4"}
...
def analyze(image=IMAGE):
 ...
 overlay=json.loads(OVERLAY.read_text());leaf=next(x for x in overlay["relocated_leaves"] if x.get("source",{}).get("path")=="components/apollo_main/core_overlay/opt3007_registers.c");patch=next(x for x in overlay["patch_sites"] if x.get("target_function")=="open_cfw_opt3007_assign_register_map")
 if leaf.get("function")!="open_cfw_opt3007_assign_register_map" or leaf.get("expected")!={"alignment":4,"offset":299528,"sha256":"37280b9f5dd6315a979b5f06cbaf7688bb18f60109d40bc9f9097df8f6ee7d15","size":224,"unrelocated_sha256":"37280b9f5dd6315a979b5f06cbaf7688bb18f60109d40bc9f9097df8f6ee7d15"} or leaf.get("relocations")!=[]:raise c.AuditError("OPT3007 relocated-leaf contract changed")
 ...
 build=json.loads(BUILD_REPORT.read_text());built=next(x for x in build["relocated_leaves"] if x.get("source",{}).get("path")=="components/apollo_main/core_overlay/opt3007_registers.c")
 validate_apollo_main_artifacts(ROOT,c.AuditError,"OPT3007")
