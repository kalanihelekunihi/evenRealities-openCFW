...
PINS={FM:'d9de48f2ee825b6a21c1fb42dd566e681f5f44f0a2d98641c2cfe22f2e4485db',PM:'d01cf53705f3e2c4f7a1a42ba12aff429458727d5543223826e6bd46980e8ad7',CL:'c78ffd05d08e3054e8b8dc61cadf8269f9c4b98a9549175ea6abc59040d9e0b1'}
SOURCE_SHA='bcfee47583acbaeb6600559c77936aa6f7cc6ba21293199aed1944785935ce7e'
...
SOURCE_OFFSETS=(312972,312976,313016,313020,313028,313036,313196,313244,313252,313256,313296,313316,313468,313480,313764,313808,313860)
...
def analyze(image=IMAGE):
 ...
 if len(source)!=18376 or sh(source)!=SOURCE_SHA:raise c.AuditError('production Ring thread source changed')
 ...
 if sum(len(x['relocations']) for x in leaves)!=72 or any(x.get('profiles')!=['apple-clang'] or x.get('strict_relocation_contract') is not True or x.get('source',{}).get('sha256')!=SOURCE_SHA or x.get('source',{}).get('license')!='GPL-3.0-only' for x in leaves):raise c.AuditError('production Ring thread source authentication changed')
 ...
 if overlay['expected']!=expected:raise c.AuditError('production Ring thread aggregate overlay pins changed')
 ...
 if (report['overlay']['size'],report['overlay']['sha256'],report['component']['size'],report['component']['sha256'])!=(332148,expected['overlay_sha256'],3855544,expected['component_sha256']) or len(built)!=17 or sum(x['placement']['size'] for x in built)!=894 or sum(x['placement']['padding_before'] for x in built)!=22:raise c.AuditError('production Ring thread build artifact changed')
 manifest=json.loads(SOURCE_MANIFEST.read_text());main=manifest['component_overrides']['apollo_main']
 if (main['provider']['size'],main['provider']['sha256'],manifest['package']['expected_size'],manifest['package']['expected_sha256'])!=(3855544,expected['component_sha256'],4634038,'3953d7a537b11d75c7f589522ae7958bd7c4f59a15d35b98d92d5bec79b90731'):raise c.AuditError('production Ring thread manifest/package pins changed')
