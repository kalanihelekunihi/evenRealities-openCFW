...
PINS={FM:'85d95f6ebb2941a76e4d7a171a737f5eda2f19d56eeba195c195eb82ffefc2cc',PM:'b18cadb1c0d563fa3bdc02a1574d5675aec88df80e25d4f457dfc9cbe7801df9',CL:'62725ab6ff3aeb1ca396d4d3e834f722f254d372baa4669eac985e9028c86950'}
SOURCE_SHA='27568beccc929897eae6e6d1fa90b8aed13fc6ccd28b27cfa038133a2b0fe839'
...
SOURCE_OFFSETS=(252164,252192,252228,252256,252304,252344,252368,252456,252484,252492,252500,252528,252536,252544,252736,252800,252820,252908)
...
def analyze(image=IMAGE):
 ...
 if len(source)!=17946 or sh(source)!=SOURCE_SHA:raise c.AuditError('production Ring service source changed')
 ...
 if sum(len(x['relocations']) for x in leaves)!=38 or any(x.get('profiles')!=['apple-clang'] or x.get('strict_relocation_contract') is not True or x.get('source',{}).get('sha256')!=SOURCE_SHA or x.get('source',{}).get('license')!='MIT' for x in leaves):raise c.AuditError('production Ring service source authentication changed')
 ...
 validate_apollo_main_artifacts(ROOT,c.AuditError,'Ring service')
 if len(built)!=18 or sum(x['placement']['size'] for x in built)!=952 or sum(x['placement']['padding_before'] for x in built)!=18:raise c.AuditError('production Ring service build artifact changed')
 manifest=json.loads(SOURCE_MANIFEST.read_text());main=manifest['component_overrides']['apollo_main']
 regions=main['regions'];owned=lambda x:x['name'].startswith('ring_service_') and x['name'][13:15].isdigit();replacement=[x for x in regions if owned(x) and '_source_replacement' in x['name']];text_regions=[x for x in regions if owned(x) and x['name'].endswith('_source_text')];alignment=[x for x in regions if owned(x) and x['name'].endswith('_overlay_alignment')];pool=[x for x in regions if x['name']=='ring_service_retained_literal_pool']
 cff_host=[x for x in regions if x.get('name','').startswith('freetype_cff_host_scatter_') and x.get('target_address') is not None and 4663692<=x['target_address']<4664160 and x.get('address_status')=='generated_source_data_replacement']
 if (len(replacement),sum(x['size'] for x in replacement),len(text_regions),sum(x['size'] for x in text_regions),len(alignment),sum(x['size'] for x in alignment),len(pool),sum(x['size'] for x in pool),len(cff_host),sum(x['size'] for x in cff_host))!=(19,1944,18,952,9,18,1,204,2,468):raise c.AuditError('production Ring service manifest ownership changed')
 leaf18=[x for x in replacement+cff_host if x.get('target_address') is not None and 4663688<=x['target_address']<4664240]
 if sorted((x['target_address'],x['target_address']+x['size']) for x in leaf18)!=[(4663688,4663692),(4663692,4664106),(4664106,4664160),(4664160,4664240)]:raise c.AuditError('production Ring service/CFF disjoint host ownership changed')
 return {'schema_version':1,'identity':{'image_sha256':c.IMAGE_SHA256,'retained_path':r'platform\protocols\ring_service\ring_service.c','embedded_third_party_definitions':[]},'surface':{'linked_functions':18,'path_anchored_functions':9,'body_bytes':2412,'physical_bytes':2616,'outer_pool_bytes':216,'reachable_instructions':927,'direct_body_calls':125,'internal_direct_body_calls':4,'external_direct_body_calls':121,'indirect_body_calls':0,'direct_bl_entry_sites':27,'stored_function_pointers':2},'provider_boundary':{'easylogger_calls':76,'cmsis_freertos_calls':2,'runtime_calls':8,'nanopb_calls':1,'cordio_calls':0,'first_party_calls':34,'historical_ring_service_commit':None,'new_version_discriminator':False},'production':{'candidate':str(SOURCE.relative_to(ROOT)),'source_inventory_available':True,'production_routed':True,'ownership_bytes':2412,'compiled_text_bytes':952,'generated_alignment_bytes':18,'strict_relocations':38,'guarded_redirects':18,'hardware_validation':'blocked by unavailable physical evidence','hardware_blocker':'hardware testing is blocked by unavailable physical evidence; future physical qualification requires a responsive authorized pair or golden Ring transport capture'}}
