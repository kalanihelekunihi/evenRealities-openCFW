...
def analyze(image=IMAGE):
 ...
 cff_host=[x for x in regions if x.get('name','').startswith('freetype_cff_host_scatter_') and x.get('target_address')==4663692 and x.get('size')==414 and x.get('address_status')=='generated_source_data_replacement']
 if (len(replacement),sum(x['size'] for x in replacement),len(text_regions),sum(x['size'] for x in text_regions),len(alignment),sum(x['size'] for x in alignment),len(pool),sum(x['size'] for x in pool),len(cff_host))!=(19,1998,18,952,9,18,1,204,1):raise c.AuditError('production Ring service manifest ownership changed')
 leaf18=[x for x in replacement+cff_host if x.get('target_address') is not None and 4663688<=x['target_address']<4664240]
 if sorted((x['target_address'],x['target_address']+x['size']) for x in leaf18)!=[(4663688,4663692),(4663692,4664106),(4664106,4664240)]:raise c.AuditError('production Ring service/CFF disjoint host ownership changed')
