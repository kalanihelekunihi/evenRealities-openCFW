#!/usr/bin/env python3
"""Register the reviewed G2 bootloader SPOT-manager transition leaf.
...
EVIDENCE = "docs/research/g2-bootloader-spotmgr-transition-428378-source-closure.md"
BOOT_BASE = 0x00410000
FUNCTION = "open_cfw_bootloader_spotmgr_transition_sequence_2b_428378"
START = 0x00428378
END = 0x004283E2
STOCK_SHA = "051e40c208a75b89a9826c46a5fcea7b9933f1de7c90f4acc01777ba1ed16866"
UNRELOCATED_SHA = "35a3808a2b454603a347435c52e5e0c1e04e37e6101a6e9948e37fde60242a8f"
DELAY = "open_cfw_bootloader_delay_us_41d1c0"
DELAY_ADDRESS = 0x0041D1C0
...
def source_record() -> dict[str, Any]:
    ...
    return {
        ...
        "origin": (
            "address-stable Apollo510 SPOT-manager transition_sequence_2b "
            "realization"
        ),
        "evidence": EVIDENCE,
...
def update_census(boot: bytes) -> None:
    ...
    for row in rows:
        ...
        if int(row["end"], 16) != END:
            raise SystemExit("SPOT-manager census interval changed")
        row.update({
            "kind": "source_function",
            "name": "spotmgr_transition_sequence_2b_428378",
            "size": str(END - START),
            "sha256": digest(boot[START - BOOT_BASE:END - BOOT_BASE]),
            "disposition": "source_owned_production",
            "provider": "AmbiqSuite Apollo510 SPOT-manager transition_sequence_2b",
        ...
        found = True
    if not found:
        raise SystemExit("SPOT-manager census interval was not found")
...
def main() -> int:
    ...
    if len(stock) != END - START or digest(stock) != STOCK_SHA:
        raise SystemExit("authenticated SPOT-manager stock body changed")
    relocation = [{
        "offset": 54,
        "type": "R_ARM_THM_CALL",
        "symbol": DELAY,
        "symbol_type": "STT_NOTYPE",
        "target_address": DELAY_ADDRESS,
    }]
    pins = {
        "size": END - START,
        "sha256": STOCK_SHA,
    ...
    update_census(boot)
    print("registered SPOT-manager transition_sequence_2b at 0x00428378")
    return 0
