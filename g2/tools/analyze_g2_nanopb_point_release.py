#!/usr/bin/env python3
"""Fail-closed nanopb 0.4.4--0.4.9.1 point-release audit for G2.
...
repository, reproduce the 0.4.7/0.4.8/0.4.9 object-code collision, and classify
the sole 0.4.9.1 runtime change as dead-stripped in G2.  The tool has no device,
debugger, serial, or flash-writing path.
"""
...
BUILD_TIMESTAMP = b"2025-04-28T13:29:15Z\0"
BUILD_TIMESTAMP_ADDRESS = 0x00768290

PB_DECODE_EX_CHANGED_CALL_TARGETS = {
    "pb_make_string_substream": 0x0048F77E,
    "pb_close_string_substream": 0x0048F7CA,
    "pb_decode_inner": 0x0048FE98,
}
PB_DECODE_EX_CHANGED_TARGET_CALLERS = {
    "pb_make_string_substream": [0x0048F9DA, 0x0048FB56, 0x0049049C],
    "pb_close_string_substream": [0x0048FA30, 0x0048FBA2, 0x00490524],
    "pb_decode_inner": [0x00490124, 0x0049051A],
}
...
UPSTREAM_RELEASES: dict[str, dict[str, Any]] = {
    ...
    "0.4.9.1": {
        ...
        "tree": "2962227caf93b83e9fd67d5d17b96fc498648dea",
        "date": "2024-12-01T12:21:56+02:00",
        "files": {
...
REFERENCE_APPLE_CLANG_OBJECTS = {
    ...
    "0.4.9.1": (
        "8e4be109e2f6816dedfbdb3b8d8a95192487d95748d594c2d971b1f6095f9185",
        "62dc1eacd8398663d95083d195ecb9e7de46928f266cb1379cba47746405afb0",
        "1b8296fe62275038936bca7dec840b5c3d898a5f6cace00dd684ca8727541c5a",
    ),
}
...
def _thumb_bl_target(address: int, first: int, second: int) -> int | None:
...
def _direct_bl_callers(data: bytes, target: int) -> list[int]:
    import struct
    ...
    for offset in range(0, len(data) - 3, 2):
        ...
        if _thumb_bl_target(address, first, second) == target:
...
def audit_image(data: bytes) -> dict[str, Any]:
    ...
    build_timestamp = _unique_address(
        data, BUILD_TIMESTAMP, BUILD_TIMESTAMP_ADDRESS, "firmware build timestamp"
    )
    changed_target_callers = {
        name: _direct_bl_callers(data, target)
        for name, target in PB_DECODE_EX_CHANGED_CALL_TARGETS.items()
    }
    if changed_target_callers != PB_DECODE_EX_CHANGED_TARGET_CALLERS:
        raise AuditError(
            "pb_decode_ex retention topology changed: "
            f"{changed_target_callers!r}"
        )
    ...
    return {
        ...
        "candidate_releases_if_pristine_upstream": [
        ...
        "firmware_build_timestamp": {
            "address": build_timestamp,
            "value": BUILD_TIMESTAMP[:-1].decode("ascii"),
            "postdates_nanopb_0_4_9_1": True,
        },
        "runtime_collision": {
        ...
        "nanopb_0_4_9_1": {
            "candidate_status": "not excluded by retained stock code",
            "source_change_from_0_4_9": (
                "pb_decode_ex records close-substream failure in status instead of "
                "returning immediately"
            ),
            "changed_function_retained": False,
            "retention_evidence": changed_target_callers,
            "qualification": (
                "The changed pb_decode_ex body is absent; the retained public pb_decode "
                "wrapper calls pb_decode_inner directly, and the retained submessage "
                "adapter is source-identical across both releases."
            ),
        },
        "upstream": {
...
def reference_build(repo: Path, clang: str) -> dict[str, Any]:
    ...
    evidence: 0.4.6 differs, 0.4.7--0.4.9 are mutually byte-identical, and
    0.4.9.1 differs only in the nonretained pb_decode_ex section.
    """
    ...
    if objects["0.4.9.1"][0] != objects["0.4.9"][0]:
        raise AuditError("0.4.9 and 0.4.9.1 pb_common objects unexpectedly differ")
    if objects["0.4.9.1"][2] != objects["0.4.9"][2]:
        raise AuditError("0.4.9 and 0.4.9.1 pb_encode objects unexpectedly differ")
    if objects["0.4.9.1"][1] == objects["0.4.9"][1]:
        raise AuditError("0.4.9 and 0.4.9.1 pb_decode objects unexpectedly collide")
    ...
    return {
        ...
        "0_4_7_through_0_4_9_objects_byte_identical": True,
        "all_candidate_release_objects_byte_identical": False,
        "stock_relevant_candidate_sections_indistinguishable": True,
        "0_4_9_1_difference_is_pb_decode_only": True,
