...
SOURCE_PIN=(35715,'8fc48108a0820ac9561dac063eeb1b0ce0e35a132d90086e3c0a67594798c0a8')
...
def analyze(image=IMAGE):
 ...
 padding=sum(x['expected']['offset']-(leaves[i-1]['expected']['offset']+leaves[i-1]['expected'].get('closure_size',leaves[i-1]['expected']['size'])) if i else x['expected']['offset']-257980 for i,x in enumerate(leaves))
 if padding!=10 or any(x.get('profiles')!=['apple-clang'] or not x.get('strict_relocation_contract') or x.get('source',{}).get('license')!='MIT' for x in leaves):raise c.AuditError('health-page production policy changed')
 ...
 validate_apollo_main_artifacts(ROOT,c.AuditError,'health page')
 ...
 repurposed={x.get('name'):x for x in main['regions'] if x.get('name') in {'liblc3_ltpf_source_rodata','liblc3_ltpf_rodata_cave_tail'}}
 repurposed_expected={'liblc3_ltpf_source_rodata':(0x4FC648,1980,'source_compiled'),'liblc3_ltpf_rodata_cave_tail':(0x4FCE04,2434,'generated_alignment')}
 if len(regions)!=37 or len(tail)!=1 or (tail[0]['target_address'],tail[0]['size'],tail[0].get('address_status'))!=(0x4FD870,8308,'official_blob') or counts!={'generated_source_entry_replacement':5000,'official_blob':432,'source_compiled':4306,'generated_alignment':10} or {k:(v.get('target_address'),v.get('size'),v.get('address_status')) for k,v in repurposed.items()}!=repurposed_expected:raise c.AuditError('health-page manifest tiling changed')
 physical_partition=counts['generated_source_entry_replacement']+counts['official_blob']+1980+2434+(PHYS[1]-0x4FD870)
 if physical_partition!=PHYS[1]-PHYS[0]:raise c.AuditError('health-page physical ownership partition changed')
 blocker='hardware testing is blocked by unavailable physical evidence; future physical qualification requires a responsive authorized G2 pair or golden health-page UI trace'
 return {'schema_version':1,'identity':{'image_sha256':c.IMAGE_SHA256,'retained_path':r'app\gui\health\ui_health_page.c','embedded_third_party_definitions':[]},'surface':{'linked_functions':12,'ghidra_discovered_functions':11,'path_anchored_functions':7,'restored_non_anchor_functions':1,'body_bytes':9414,'physical_bytes':10054,'outer_pool_bytes':756,'reachable_instructions':3235,'direct_body_calls':678,'internal_direct_body_calls':12,'external_direct_body_calls':666,'indirect_body_calls':0,'direct_bl_entry_sites':19,'stored_function_pointers':2,'classified_noncode_pseudo_bl_sites':1},'provider_boundary':{'lvgl_calls':437,'easylogger_calls':55,'cmsis_freertos_calls':0,'runtime_calls':4,'mpaland_printf_calls':36,'first_party_calls':134,'historical_health_page_commit':None,'new_version_discriminator':False},'behavior':{'two_page_health_view':True,'health_metric_formatting':True,'goal_progress_scaling':True,'deferred_fifo_events':True,'animated_page_switching':True,'refresh_and_teardown':True},'production':{'source_inventory_available':True,'production_routed':True,'candidate':SOURCE_PATH,'source_functions':12,'compiled_text_bytes':3978,'compiled_rodata_bytes':328,'generated_alignment_bytes':10,'guarded_redirects':12,'stock_function_bytes_redirected':9414,'generated_redirect_bytes':5000,'repurposed_liblc3_source_rodata_bytes':1980,'repurposed_generated_cave_bytes':2434,'retained_compatibility_bytes':640,'strict_relocations':269,'hardware_validation':'blocked by unavailable physical evidence','hardware_blocker':blocker}}
