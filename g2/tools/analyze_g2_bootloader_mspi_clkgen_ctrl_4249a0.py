...
def audit():
 ...
 successor=by_name["bootloader_mspi_initialize_424a5a_source_in_place"]
 ...
 return {"status":"production-routed-exact-dual-profile-source / hardware-validation-deferred-by-project-direction","callers":list(callers),"profiles":profiles,"production":{"routed":True,"source_owned_bytes":c["source_owned_bytes"],"retained_official_bytes":c["opaque_base_bytes"],"next_frontier":successor["target_address"]},"next_frontier":{"start":0x424A5A,"end":0x424AEA,"identity":"am_hal_mspi_initialize","bytes":144},"hardware_validation":"deferred by project direction","hardware_operations":[]}
def main():
 p=argparse.ArgumentParser();p.add_argument("--json",action="store_true");r=audit();print(json.dumps(r,indent=2,sort_keys=True) if p.parse_args().json else "Bootloader clock/XIP-delay: exact production source; hardware validation deferred by project direction");return 0
