#!/usr/bin/env python3
"""Register the G2 stored-pointer event dispatcher."""
...
EVIDENCE = "docs/research/g2-bootloader-event-dispatch-42f38e-source-closure.md"
...
UNRELOCATED_SHA = "cd4e930e11246ab861c5a68f3ef0bfbd03315e7f51d01633b961a9e81ad7f92b"
SOURCE_SHA = "5f1102f84082416beaf905c89732103956a38e8b5776961cb836246237bda295"
...
def update_census() -> None:
    ...
    if len(matches) != 1 or int(matches[0]["end"], 16) != END:
        raise SystemExit("event-dispatch census boundary changed")
    matches[0].update({"kind": "source_function", "name": "event_dispatch_42f38e",
                       "disposition": "source_owned_production",
                       "provider": "clean-room stored-pointer event dispatcher",
                       "license_status": "MIT",
                       "evidence": "exact dual-toolchain and Apollo-main body; stored ingress and provider calls pinned"})
...
def main() -> int:
    ...
    if len(source) != 3_328 or digest(source) != SOURCE_SHA:
        raise SystemExit("event-dispatch source identity changed")
    if digest(boot[START - BOOT_BASE:END - BOOT_BASE]) != BODY_SHA:
        raise SystemExit("authenticated event-dispatch body changed")
    source_record = {"path": SOURCE.relative_to(ROOT).as_posix(),
                     "size": len(source), "sha256": SOURCE_SHA, "license": "MIT",
                     "origin": "clean-room stored-pointer event dispatcher",
                     "evidence": EVIDENCE}
    relocations = [
    ...
    print("registered stored-pointer event dispatcher")
    return 0
