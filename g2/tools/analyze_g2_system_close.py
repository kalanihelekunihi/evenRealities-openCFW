...
LEAF_DIGEST='430c866c27243881080960bdd31da257a01720b58dd1f3d3973e275239ba3505';PATCH_DIGEST='004cb946fa5723a6505b706f20ad27443e7fe98ebb67713c08ce140a1685b64a';BUILT_DIGEST='63d93d17ec939407923a0a22bed5a45e14a429b8ddb6310ec0cf23543d16905d';REGION_DIGEST='d4003b0006936070f9134b9e3cc71285fd882848af31f7cd30728aac4b2a138f'
...
def analyze(image=IMAGE):
 ...
 manifest=json.loads(MANIFEST.read_text());main=manifest['component_overrides']['apollo_main'];regions=[x for x in main['regions'] if x['name'].startswith('system_close_') or (x.get('name','').startswith('freetype_cff_host_scatter_') and x.get('target_address')==4631276 and x.get('size')==464)]
 if len(regions)!=59 or jsh(regions)!=REGION_DIGEST or sum(x['size'] for x in regions)!=8194:raise c.AuditError('production SystemClose manifest regions changed')
 ...
 if counts!={'generated_source_entry_replacement':(21,4496),'official_blob':(6,408),'source_compiled':(20,2804),'generated_alignment':(11,22)}:raise c.AuditError('production SystemClose stock/overlay tiling changed')
 ...
 if len(host)!=1 or sorted((x['target_address'],x['target_address']+x['size']) for x in leaf17)!=[(4631272,4631276),(4631276,4631740),(4631740,4631748)]:raise c.AuditError('production SystemClose/CFF disjoint host ownership changed')
