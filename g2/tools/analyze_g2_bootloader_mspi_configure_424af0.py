...
def audit():
 ...
 regions=json.loads(MANIFEST.read_text())["component_overrides"]["apollo_bootloader"]["regions"];by_name={x["name"]:x for x in regions};routed=by_name["bootloader_mspi_configure_424af0_source_in_place"];require((routed["target_address"],routed["size"],routed["address_status"])==(ENTRY,COMPILED_SIZE,"source_compiled"),"source configure boundary changed");retained=by_name["bootloader_mspi_device_config_literals_424bd4_opaque"];require((retained["target_address"],retained["size"],retained["address_status"])==(ENTRY+COMPILED_SIZE,92,"official_blob"),"retained configure literal boundary changed")
