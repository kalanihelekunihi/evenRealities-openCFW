...
The clean-room G2 queue candidate now closes pool construction, dummy reset,
allocation, recycling, enqueue, dequeue, and one-shot state initialization for
the complete fixed-record queue lifecycle. A second clean-room candidate closes
both callback setters, the default-metadata setter, and the bounded record
drain. A third closes the CMSIS event-worker/thread orchestration while keeping
all first-party persistence handlers explicit. All three are intentionally
separate from this MIT snapshot and remain production-excluded pending target
concurrency and hardware stress. Image-specific transport ownership is the
remaining downstream boundary; no `elog_async_api.c` algorithm is still opaque.
