# CMSIS-FreeRTOS v10.5.1 authenticated source snapshot
...
This snapshot is the authenticated source/header baseline for the complete
stock linked CMSIS wrapper object. The separate linked-function census maps 43
functions / 3,758 executable bytes plus 22 literal bytes in the 3,780-byte
object, including 38 public APIs and five private helpers. It also bounds the
live history to the v10.5.1 lineage and records that this exact source blob was
first introduced by `13acfbef7be85119fc6bc56832c455d4547d92c7`.

Sixteen bounded public algorithms—`osMessageQueueNew`, `osMutexNew`,
`osSemaphoreNew`, `osKernelGetTickCount`, `osThreadGetId`,
...
`osMutexRelease`, and `osSemaphoreRelease`—plus private `IRQ_Context` are
production-integrated through separate adapters. The pristine snapshot itself
is not linked wholesale, and the remaining 22 public
APIs and four private helpers are not production-owned. Recovered FreeRTOS
configuration, the Even-extended TCB, ABI/layout behavior, hardware hooks,
relocations, and fixed-address integration remain separate review gates.
