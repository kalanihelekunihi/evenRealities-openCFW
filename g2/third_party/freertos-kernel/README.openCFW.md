...
That compatibility change now exists as the separately authenticated minimal
patch `components/shared/freertos/g2-tcb-v10.5.1.patch`; it remains outside
this pristine vendor tree and target-layout compiles to the exact `0x70`-byte
ABI. The vendor's original field name and private patch commit remain
unobservable.
...
`docs/research/freertos-g2-config-port-audit.md` and
