...
def verify_g2_config(provenance: dict[str, Any]) -> None:
    ...
    sysenv = boundary["default_kv_tables"][0]
    boot = sysenv["runtime_zero_value_recovered"]
    require(sysenv["initialized_values_recovered"] == 12, "sysenv default closure changed")
    require(
        boot
        == {
            "key": "kvbooCount",
            ...
            "lifecycle": "read persisted word, increment, write persisted word",
        },
        "boot-count lifecycle changed",
    )
    require(
        boundary["first_party_reset_policy"]["versioned_key_migration_callbacks"] == 11,
        "first-party migration callback count changed",
    )
...
def verify_g2_image() -> None:
    ...
    require(
        sha256(image_slice(image, 0x0075D3C8, 0x0075D3E0))
        == "2a52162870b1f1b036f1769e75dabcf475f17eb6c37c85ffcabcb9dcfd064e15",
        "startup zero record changed",
    )
    require(
        sha256(image_slice(image, 0x00746D20, 0x00746D4C))
        == "bd1d3336c4e37373945074a222e8bc34f71269373b0cf16ffe7fbd7bc779b112",
        "KV migration table changed",
    )
    binding_pointers = {
