 ...
 *
 * Production clean-room source recovery of Apollo main's private G2 EasyLogger
 * level-aware record builder at 0x00448D4E...0x00448DD1.
 *
 * This implementation deliberately preserves stock failure ownership, including
 * the second recycle after retained enqueue has already recycled on retry
 * exhaustion.  It is G2 transport glue, not upstream EasyLogger code.
 */
...
extern void open_cfw_retained_easylogger_async_record_recycle(
...
)
{
    ...
    if (
        ...
        /*
         * Exact stock compatibility: retained enqueue already recycles on
         * its retry-limit failure, and the stock builder recycles again.
         */
