...
#include "wsf_trace.h"
#include "runtime_cordio_dm_conn_wsf_msg.h"
#include "wsf_os.h"
#include "util/bstream.h"
#include "dm_api.h"
#include "runtime_cordio_dm_conn_dm_dev.h"
#include "dm_main.h"
#include "runtime_cordio_dm_conn.h"

#ifdef OPEN_CFW_DM_CONN_PRODUCTION
#define dmConnCb (*(dmConnCb_t *)(uintptr_t) 0x200712A4U)
#define dmConnUpdActSet ((dmConnAct_t **)(uintptr_t) 0x20073FD8U)
#define dmConnActSet ((dmConnAct_t **)(uintptr_t) 0x20073FE4U)
#define dmFcnIfTbl ((dmFcnIf_t **)(uintptr_t) 0x20000694U)
#define dmCb (*(dmCb_t *)(uintptr_t) 0x20073B78U)
#endif
...
#define DM_CONN_ID_VALID(connId)     (((connId) > DM_CONN_ID_NONE) && ((connId) <= DM_CONN_MAX))
#define DM_CONN_APP_CALLBACK(event) do { \
  dmCback_t open_cfw_dm_conn_callback = dmConnCb.connCback[DM_CLIENT_ID_APP]; \
  if (open_cfw_dm_conn_callback != NULL) \
  { \
    open_cfw_dm_conn_callback((dmEvt_t *)(event)); \
  } \
} while (0)
...
#ifndef OPEN_CFW_DM_CONN_PRODUCTION
/* Conn spec defaults */
...
#else
#define dmConnSpecDefaults (*(const hciConnSpec_t *)(uintptr_t) 0x0078A814U)
#define dmConnActSetMain ((const dmConnAct_t *)(uintptr_t) 0x00776A84U)
#define dmConnUpdActSetMain ((const dmConnAct_t *)(uintptr_t) 0x0078EFECU)
#define dmConnFcnIf (*(const dmFcnIf_t *)(uintptr_t) 0x0078A820U)
#define dmConn2FcnIf (*(const dmFcnIf_t *)(uintptr_t) 0x0078A82CU)
#define dmConnUpdFcnIf (*(const dmFcnIf_t *)(uintptr_t) 0x0078A838U)
#define dmConnUpdActTbl ((const uint8_t *)(uintptr_t) 0x0078EFF0U)
#endif
                            ...
                            uint8_t role)
{
  ...
  if ((clientId >= DM_CLIENT_ID_MAX) || (pAddr == NULL) ||
      ((role != DM_ROLE_MASTER) && (role != DM_ROLE_SLAVE)))
  {
...
void dmConnUpdExecute(dmConnCcb_t *pCcb, dmConnMsg_t *pMsg)
{
  ...
  /* get action */
  ordinal = DM_MSG_MASK(pMsg->hdr.event);
  if (ordinal >= DM_CONN_UPD_NUM_MSGS)
  {
    dmConnUpdActNone(pCcb, pMsg);
    return;
  }
  action = dmConnUpdActTbl[ordinal];

  if ((DM_CONN_ACT_SET_ID(action) >= DM_CONN_NUM_ACT_SETS) ||
      (DM_CONN_ACT_ID(action) >= 4U))
  {
    dmConnUpdActNone(pCcb, pMsg);
    return;
  }
...
void DmConnClose(uint8_t clientId, dmConnId_t connId, uint8_t reason)
{
  ...
  if ((clientId >= DM_CLIENT_ID_MAX) || !DM_CONN_ID_VALID(connId))
  {
    return;
  }
...
void DmConnUpdate(dmConnId_t connId, hciConnSpec_t *pConnSpec)
{
  ...
  if (!DM_CONN_ID_VALID(connId) || (pConnSpec == NULL))
  {
    return;
  }
...
void DmExtConnSetScanInterval(uint8_t initPhys, uint16_t *pScanInterval, uint16_t *pScanWindow)
{
  ...
  if ((pScanInterval == NULL) || (pScanWindow == NULL))
  {
    return;
  }
...
/*************************************************************************************************/
/*!
 *  \brief  Initialize the three product connection records before Cordio starts.
 *
 *  This is the source-owned form of the adjacent G2 product helper at
 *  0x004B73E8.  The explicit assignments preserve its authenticated field
 *  defaults while avoiding any dependency on compiler-generated padding.
 */
/*************************************************************************************************/
void vendorCcbInitLike(void)
{
  ...
  {
    dmConnCcb_t *pCcb = &dmConnCb.ccb[i];
