...
#ifndef OPEN_CFW_RUNTIME_FREERTOS_CLI_CONSOLE_H
#define OPEN_CFW_RUNTIME_FREERTOS_CLI_CONSOLE_H
...
/*
 * Exact retained call ABI used at 0x005416A0.  Production accepts a byte only
 * when the returned count is exactly one.  Stock ignored this return and
...
/*
 * Internal source-leaf seams, exposed so each function remains placeable.
 * The candidate's fill helper has exactly one live tuple: 128 bytes of zero.
 * Closing that tuple in this interface preserves the authenticated console
 * behavior and the candidate compiler's independently emitted clear leaf.
 */
...
/* One bounded receive/consume iteration, also suitable for host tests. */
open_cfw_freertos_cli_console_u32
open_cfw_freertos_cli_console_poll_once(
    struct open_cfw_freertos_cli_console_state *state
);

/* CMSIS/FreeRTOS thread-entry ABI selected by stored pointer 0x00541601. */
