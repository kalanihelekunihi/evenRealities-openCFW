 ...
 *
 * Altered, production-excluded source candidate for nanopb
 * pb_make_string_substream(). The selected baseline is authenticated nanopb
 * 0.4.9 commit 98bf4db69897b53434f3d0ba72e0a3ab1a902824. Existing binary
 * evidence narrows the vendor runtime only to the source-equivalent
 * 0.4.7--0.4.9 interval.
 *
 * The complete stock body occupies [0x0048F77E, 0x0048F7CA). Stock copies the
 * recovered 16-byte stream object through the aligned interior entry of the
 * compiler's __aeabi_memcpy. This candidate closes that seam with four named
 * ABI-field assignments. It remains absent from every production overlay and
 * manifest pending target-object, relocation, placement, and ownership review.
 */
...
bool open_cfw_nanopb_make_string_substream_candidate(
    struct open_cfw_nanopb_istream *stream,
