 ...
 *
 * Altered production source for nanopb read_raw_value(). The selected
...
static const struct {
    char varint_overflow[16];
    char invalid_wire_type[18];
} open_cfw_nanopb_raw_errors = {
    "varint overflow",
    "invalid wire_type"
};
...
)
{
            ...
            *size += 1U;
            if (*size > maximum_size) {
                return open_cfw_nanopb_raw_error(
                    stream,
                    open_cfw_nanopb_raw_errors.varint_overflow
                );
            }
    ...
    default:
        return open_cfw_nanopb_raw_error(
            stream,
            open_cfw_nanopb_raw_errors.invalid_wire_type
        );
    }
}
