...
#ifndef OPEN_CFW_RUNTIME_FREERTOS_CLI_GET_PARAMETER_H
#define OPEN_CFW_RUNTIME_FREERTOS_CLI_GET_PARAMETER_H

/*
 * Isolated 32-bit ABI for the production FreeRTOS+CLI parameter accessor.
 * These types match the authenticated G2 ABI without importing the rest of
 * the FreeRTOS application configuration.
 */
 ...
 * be NUL terminated.  The production console-capacity patch independently
 * enforces that precondition for the G2 command collector.
 */
const char *open_cfw_freertos_cli_get_parameter(
