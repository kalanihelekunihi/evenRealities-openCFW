...
class NanopbDecodeVarintProductionTests(unittest.TestCase):
    ...
    def test_registration_patch_neighbors_and_whole_component_ingress(self) -> None:
        config = json.loads(OVERLAY_CONFIG.read_text(encoding="utf-8"))
        self.assertEqual(len(config["functions"]), 664)
        self.assertEqual(len(config["patch_sites"]), 613)
        self.assertEqual(len(config["relocated_leaves"]), 95)
        ...
        self.assertEqual(
            direct_bl,
            [
                address for address, _ in candidate_contract.CALLERS
                if address != 0x0049_0156
            ],
        )
    ...
    def test_manifest_tiling_ownership_package_and_notices_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            {
                "expected_size": 4_428_972,
                "expected_sha256": "22117e0cd7d0b827a8c31d22eb509edb30651fef6a6308838a8220ff80f6c702",
            },
        )
        for path, tokens in {
