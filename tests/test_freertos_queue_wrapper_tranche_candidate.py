...
CANDIDATE = (
    ...
    / "research"
    / "candidates"
    / "freertos_queue_wrapper_tranche.c"
)
FIXTURE = (
    ROOT
    / "tests"
    / "fixtures"
    / "freertos_queue_wrapper_tranche_candidate_host.c"
)
...
class FreeRTOSQueueWrapperTrancheCandidateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        library = temporary / (
            "freertos_queue_wrapper_tranche_candidate.dylib"
            if sys.platform == "darwin"
            else "freertos_queue_wrapper_tranche_candidate.so"
        )
        host_command = [
    ...
    def test_official_bodies_and_upstream_snapshot_are_authenticated(
        self,
    ) -> None:
    ...
    def test_static_mutex_forwards_type_and_initialises_even_on_failure(
        self,
    ) -> None:
        self.reset()
        ...
        self.assertEqual(self.uint(
            "open_cfw_test_queue_wrapper_static_create_calls"), 1)
        self.assertEqual(self.uint(
            "open_cfw_test_queue_wrapper_initialise_mutex_calls"), 1)
        self.assertEqual(self.uint("open_cfw_test_queue_wrapper_length"), 1)
        self.assertEqual(
            self.uint("open_cfw_test_queue_wrapper_item_size"), 0)
        self.assertEqual(
            self.uint("open_cfw_test_queue_wrapper_queue_type"), 4)
        self.assertEqual(
            self.ulong("open_cfw_test_queue_wrapper_storage"), 0)
        self.assertEqual(
        ...
        self.assertEqual(self.uint(
            "open_cfw_test_queue_wrapper_initialise_mutex_calls"), 1)
        self.assertEqual(
            self.ulong("open_cfw_test_queue_wrapper_mutex_argument"), 0)
        self.assertEqual(
            self.uint("open_cfw_test_queue_wrapper_assert_calls"), 0)

    def test_static_counting_success_failure_and_arguments(self) -> None:
        self.reset()
        queue = self.static_pointer()
        self.assertEqual(self.counting_static(7, 5, queue), queue)
        self.assertEqual(self.uint(
            "open_cfw_test_queue_wrapper_static_create_calls"), 1)
        ...
        self.assertEqual(
            self.uint("open_cfw_test_queue_wrapper_item_size"), 0)
        self.assertEqual(
            self.uint("open_cfw_test_queue_wrapper_queue_type"), 2)
        self.assertEqual(
            self.ulong("open_cfw_test_queue_wrapper_storage"), 0)
        self.assertEqual(self.messages(queue), 5)

        self.reset()
        before = self.messages(queue)
        self.set_uint("open_cfw_test_queue_wrapper_static_succeeds", 0)
        self.assertIsNone(self.counting_static(7, 5, queue))
        self.assertEqual(self.messages(queue), before)
        self.assertEqual(
            self.uint("open_cfw_test_queue_wrapper_assert_calls"), 0)

    def test_dynamic_counting_success_failure_and_unsigned_boundary(
        self,
    ) -> None:
        self.reset()
        queue = self.dynamic_pointer()
        self.assertEqual(self.counting_dynamic(9, 9), queue)
        self.assertEqual(self.uint(
            "open_cfw_test_queue_wrapper_dynamic_create_calls"), 1)
        self.assertEqual(self.uint("open_cfw_test_queue_wrapper_length"), 9)
        self.assertEqual(
            self.uint("open_cfw_test_queue_wrapper_item_size"), 0)
        self.assertEqual(
            self.uint("open_cfw_test_queue_wrapper_queue_type"), 2)
        self.assertEqual(self.messages(queue), 9)

        self.reset()
        self.assertEqual(
            self.counting_dynamic(0xFFFFFFFF, 0xFFFFFFFF),
            queue,
        )
        self.assertEqual(self.messages(queue), 0xFFFFFFFF)
    ...
    def test_both_counting_wrappers_use_unsigned_high_bit_ordering(
        self,
    ) -> None:
        queue = self.static_pointer()
        ...
        for maximum, initial in (
            ...
            self.reset()
            self.assertEqual(
                self.counting_static(maximum, initial, queue),
                queue,
            )
            self.assertEqual(self.messages(queue), initial)
            self.assertEqual(
            ...
            self.assertEqual(self.messages(dynamic), initial)
            self.assertEqual(
                self.uint("open_cfw_test_queue_wrapper_assert_calls"),
                0,
            )
        ...
        self.reset()
        self.assertIsNone(
            self.counting_static(0x7FFFFFFF, 0x80000000, queue)
        )
        self.assertEqual(
        ...
        self.reset()
        self.assertIsNone(
            self.counting_dynamic(0x7FFFFFFF, 0x80000000)
        )
        self.assertEqual(
    ...
    def test_invalid_counting_dimensions_use_only_assertion_seam(
        self,
    ) -> None:
        for maximum, initial in ((0, 0), (0, 1), (4, 5)):
            self.reset()
            self.assertIsNone(
                self.counting_static(
                    maximum,
                    initial,
                    self.static_pointer(),
                )
            ...
            self.assertEqual(self.uint(
                "open_cfw_test_queue_wrapper_static_create_calls"), 0)
            self.assertEqual(
                self.uint("open_cfw_test_queue_wrapper_assert_calls"), 1)
            ...
            self.assertEqual(self.uint(
                "open_cfw_test_queue_wrapper_dynamic_create_calls"), 0)
            self.assertEqual(
                self.uint("open_cfw_test_queue_wrapper_assert_calls"), 1)

    def test_candidate_scope_and_target_relocation_surface_are_bounded(
        self,
    ) -> None:
        ...
        defined_globals = {
            ...
            and (self.symbols[name][3] >> 4) == 1
        }
        self.assertEqual(defined_globals, TARGET_NAMES)
        ...
        self.assertEqual(
            [
                ...
                for relocation in metadata_relocations
            ],
            [
                (".ARM.exidx", 42, ""),
                (".ARM.exidx", 42, ""),
        ...
        mutex_body = self.target_function_bytes(
            "open_cfw_freertos_queue_create_mutex_static"
        )
        self.assertIn(bytes.fromhex("c4b2"), mutex_body)
        self.assertNotIn(
            assertion_call,
            mutex_body,
        )
