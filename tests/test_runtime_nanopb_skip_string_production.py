...
BASE_FUNCTION_COUNT = 663
BASE_RELOCATED_COUNT = 94
BASE_PATCH_COUNT = 612
...
PRODUCTION_BUILD_PROFILES = {
    "apple-clang": {
        ...
        "census": (1_032, 2, 5),
        "effective_ownership": (126_028, 88_659, 4_212_461),
    },
    "linux-clang": {
        "boot_component": (
            149_262,
        ...
        "census": (848, 2, 5),
        "effective_ownership": (127_963, 88_548, 4_212_461),
    },
}
...
OVERLAY_MUTATION_RECORDS = (
    ...
    ("extra_pb_skip_field_leaf", "pb_skip_field promotion is forbidden", "config"),
...
def apply_overlay_mutation(
    ...
    elif name == "extra_pb_skip_field_leaf":
        duplicate = copy.deepcopy(leaf)
        duplicate["function"] = "open_cfw_nanopb_skip_field"
        config["relocated_leaves"].append(duplicate)
        config["functions"].append(duplicate["function"])
    else:
        raise KeyError(
...
def validate_skip_string_task3_preflight(
    ...
    *,
    ...
    if any(
        item.get("function") == "open_cfw_nanopb_skip_field"
        for item in config["relocated_leaves"]
    ) or any(
        item.get("target_function") == "open_cfw_nanopb_skip_field"
        for item in config["patch_sites"]
    ):
        raise AssertionError("pb_skip_field promotion is forbidden")
    ...
    if leaf_functions[-1] != PRODUCTION_FUNCTION or leaf_functions[-2] != (
...
class NanopbSkipStringProductionContractTests(StableUnittestIdentityCase):
    ...
    def _assert_registered_overlay_and_builder_contract(
        ...
        self.assertEqual(config["relocated_leaves"][-1]["function"], PRODUCTION_FUNCTION)
        ...
        with tempfile.TemporaryDirectory(
            ...
            self.assertEqual(
                component[caller_offset:caller_offset + 4], CALLER_ENCODING
            )
            self.assertEqual(
    ...
    def test_manifest_splits_stock_span_and_appends_leaf(self) -> None:
        ...
        expected_records = {
            ...
            "opaque_application_between_nanopb_skip_string_and_nanopb_close_string_substream": (
                ...
                0x0048_F66C,
                "official_blob",
                "apollo510b/main-opaque-0x0048f66c.bin",
            ),
        ...
        self.assertEqual(
            ...
            {
                "container_only": (1, 32),
                "generated_alignment": (54, 108),
                "generated_source_entry_replacement": (599, 86_572),
                "generated_source_exact_load_image": (1, 6),
                "generated_source_exact_replacement": (7, 134),
                "official_blob": (183, 3_436_434),
                "source_compiled": (115, 125_368),
            },
        )
        ...
        self.assertEqual(
            ...
            "442fff1ab900ea02b8b1d997588af3811e7e674bdb405167d90c6d94e13157d0",
        )
        self.assertEqual(
        ...
        historical_main_provider = historical_manifest[
            "component_overrides"
        ]["apollo_main"]["provider"]
        historical_main_pins = (
            open_cfw.profile_pins(historical_main_provider, profile)
            or historical_main_provider
        )
        rollback_raw = bytearray(
            component[PACKAGE_PREAMBLE:historical_main_pins["size"]]
        )
        ...
        self.assertEqual(
            (len(rollback_component), sha256(rollback_component)),
            (historical_main_pins["size"], historical_main_pins["sha256"]),
        )
        merged = open_cfw.load_manifest(MANIFEST)
        payloads = {
            component_record["name"]: open_cfw.resolve_below(
                ROOT, component_record["provider"]["path"]
            ).read_bytes()
            for component_record in merged["components"]
        }
        payloads["apollo_main"] = rollback_component
        rollback_package, _ = open_cfw.assemble_evenota(merged, payloads)
        historical_package = historical_manifest["package"]
        historical_package_pins = (
            open_cfw.profile_pins(historical_package, profile)
            or historical_package
        )
        self.assertEqual(
            (len(rollback_package), sha256(rollback_package)),
            (
                historical_package_pins["expected_size"],
                historical_package_pins["expected_sha256"],
            ),
        )
        ...
        if profile == "apple-clang":
            self.assertEqual(
                plan_ownership,
                {
                    ...
                    "generated_alignment": (56, 111),
                    "generated_source_entry_replacement": (631, 87_432),
                    "generated_source_exact_load_image": (1, 6),
                    "generated_source_exact_replacement": (7, 134),
                    "inferred_from_vector_table": (1, 34_432),
                    "official_blob": (194, 3_584_173),
                    "source_compiled": (137, 126_028),
                    "unknown": (2, 326_044),
                },
            )
