...
class EasyLoggerOutputCandidateTests(unittest.TestCase):
    ...
    def test_audited_candidate_is_the_exact_production_closure(self) -> None:
        ...
        leaves = {
            leaf["function"]: leaf
            for leaf in overlay["relocated_leaves"][-3:]
        }
        self.assertTrue(leaves[FUNCTION]["strict_relocation_contract"])
        self.assertEqual(leaves[FUNCTION]["expected"]["offset"], 119_520)
        self.assertEqual(
        ...
        sites = {
            ...
            for site in overlay["patch_sites"][-3:]
        }
        ...
        appended = next(
            region for region in regions
            if region.get("target_address") == 0x007B_1604
        )
