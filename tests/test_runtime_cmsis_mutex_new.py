...
PRODUCTION_FLASH_PLAN_SHA256 = (
    "06bbb5f11b96268fb6803edfed8e4204"
    "78a7995ebfab4ab900fe84126396dbe6"
)
...
class RuntimeCmsisMutexNewTests(unittest.TestCase):
    ...
    def test_production_config_report_redirect_and_artifacts_are_exact(
        ...
        self.assertEqual(component_report["size"], 3_645_102)
        self.assertEqual(
        ...
        self.assertEqual(component_report["source_owned_bytes"], 121_900)
        self.assertEqual(component_report["opaque_base_bytes"], 3_438_528)

        overlay = CURRENT_OVERLAY.read_bytes()
        component = CURRENT_COMPONENT.read_bytes()
        package = CURRENT_PACKAGE.read_bytes()
        self.assertEqual(len(overlay), 121_706)
        self.assertEqual(sha256(overlay), PRODUCTION_OVERLAY_SHA256)
        self.assertEqual(len(component), 3_645_102)
        self.assertEqual(sha256(component), PRODUCTION_COMPONENT_SHA256)
        self.assertEqual(len(package), 4_423_556)
        ...
        self.assertEqual(len(flash_plan), 646_934)
        self.assertEqual(
            sha256(flash_plan),
            PRODUCTION_FLASH_PLAN_SHA256,
        )
        parsed_plan = json.loads(flash_plan)
        self.assertEqual(len(parsed_plan["flash_regions"]), 927)
        self.assertEqual(
