from __future__ import annotations

import os

import ctypes
import hashlib
import random
import subprocess
import sys
import tempfile
import unittest
from pathlib import Path


OPENCFW_ROOT = Path(__file__).resolve().parent.parent
COMPONENT_ROOT = OPENCFW_ROOT / "components" / "apollo_main" / "core_overlay"
SOURCE = COMPONENT_ROOT / "pwrctrl_syspll_disable.c"
FIXTURE = (
    OPENCFW_ROOT / "tests" / "fixtures" / "pwrctrl_syspll_disable_host.c"
)

CHIPREV = 0x4002000C
D2ASPARE = 0x400201B0
PLLCTL0 = 0x400204D8
ISOLATION = 0x00200000
VDDF_POWER_DOWN = 0x80000000
VDDH_POWER_DOWN = 0x40000000
UINT_MASK = 0xFFFFFFFF


class PwrctrlSyspllDisableTests(unittest.TestCase):
    IRQ_DISABLE = 1
    READ = 2
    WRITE = 3
    DELAY = 4
    IRQ_RESTORE = 5

    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        library = Path(cls.temporary.name) / (
            "pwrctrl_syspll_disable.dylib"
            if sys.platform == "darwin"
            else "pwrctrl_syspll_disable.so"
        )
        command = [
            os.environ.get("OPENCFW_CLANG", "/usr/bin/clang"),
            "-O2",
            "-Wall",
            "-Wextra",
            "-Werror",
            str(FIXTURE),
        ]
        if sys.platform == "darwin":
            command.extend(["-dynamiclib", "-o", str(library)])
        else:
            command.extend(["-shared", "-fPIC", "-o", str(library)])
        subprocess.run(command, check=True, capture_output=True, text=True)
        cls.loaded = ctypes.CDLL(str(library))
        cls.reset_fixture = cls.loaded.open_cfw_test_syspll_disable_reset
        cls.reset_fixture.argtypes = [ctypes.c_uint] * 5
        cls.reset_fixture.restype = None
        cls.disable = cls.loaded.open_cfw_pwrctrl_syspll_disable
        cls.disable.argtypes = []
        cls.disable.restype = ctypes.c_uint

    @classmethod
    def tearDownClass(cls) -> None:
        cls.temporary.cleanup()

    @classmethod
    def word(cls, name: str) -> ctypes.c_uint:
        return ctypes.c_uint.in_dll(
            cls.loaded,
            f"open_cfw_test_syspll_disable_{name}",
        )

    @classmethod
    def word_array(cls, name: str) -> ctypes.Array:
        return (ctypes.c_uint * 32).in_dll(
            cls.loaded,
            f"open_cfw_test_syspll_disable_trace_{name}",
        )

    def reset(
        self,
        revision: int,
        d2aspare: int,
        pllctl0: int,
        primask: int = 0,
        injected_pll_bits: int = 0,
    ) -> None:
        self.reset_fixture(
            revision,
            d2aspare,
            pllctl0,
            primask,
            injected_pll_bits,
        )

    def trace(self) -> list[tuple[int, int, int]]:
        count = min(self.word("trace_count").value, 32)
        events = self.word_array("event")
        addresses = self.word_array("address")
        values = self.word_array("value")
        return [
            (events[index], addresses[index], values[index])
            for index in range(count)
        ]

    def test_pre_b1_skips_isolation_and_delay(self) -> None:
        self.reset(
            revision=0xABCD0021,
            d2aspare=0xA5A5A5A5,
            pllctl0=0,
            primask=1,
        )

        self.assertEqual(self.disable(), 0)
        self.assertEqual(self.word("d2aspare").value, 0xA5A5A5A5)
        self.assertEqual(self.word("pllctl0").value, 0xC0000000)
        self.assertEqual(self.word("restored_primask").value, 1)
        self.assertEqual(
            self.trace(),
            [
                (self.IRQ_DISABLE, 0, 1),
                (self.READ, PLLCTL0, 0),
                (self.WRITE, PLLCTL0, 0x80000000),
                (self.READ, PLLCTL0, 0x80000000),
                (self.WRITE, PLLCTL0, 0xC0000000),
                (self.READ, CHIPREV, 0xABCD0021),
                (self.IRQ_RESTORE, 0, 1),
            ],
        )

    def test_b1_threshold_uses_only_low_revision_byte(self) -> None:
        for revision, expected_isolation in (
            (0x00000022, True),
            (0xFFFFFF22, True),
            (0x00000121, False),
        ):
            with self.subTest(revision=revision):
                self.reset(
                    revision=revision,
                    d2aspare=0,
                    pllctl0=0,
                )
                self.assertEqual(self.disable(), 0)
                self.assertEqual(
                    bool(self.word("d2aspare").value & ISOLATION),
                    expected_isolation,
                )
                delays = [
                    value
                    for event, _, value in self.trace()
                    if event == self.DELAY
                ]
                self.assertEqual(delays, [1] if expected_isolation else [])

    def test_b1_path_has_exact_register_and_critical_section_order(self) -> None:
        self.reset(
            revision=0x22,
            d2aspare=0xA5A5A5A5,
            pllctl0=0x01234567,
            primask=0xA5A5A5A5,
        )

        self.assertEqual(self.disable(), 0)
        self.assertEqual(
            self.trace(),
            [
                (self.IRQ_DISABLE, 0, 0xA5A5A5A5),
                (self.READ, PLLCTL0, 0x01234567),
                (self.WRITE, PLLCTL0, 0x81234567),
                (self.READ, PLLCTL0, 0x81234567),
                (self.WRITE, PLLCTL0, 0xC1234567),
                (self.READ, CHIPREV, 0x22),
                (self.DELAY, 0, 1),
                (self.READ, D2ASPARE, 0xA5A5A5A5),
                (self.WRITE, D2ASPARE, 0xA5A5A5A5 | ISOLATION),
                (self.IRQ_RESTORE, 0, 0xA5A5A5A5),
            ],
        )

    def test_unrelated_register_bits_are_preserved(self) -> None:
        self.reset(
            revision=0xFF,
            d2aspare=0xA5A5A5A5,
            pllctl0=0x12345678,
        )

        self.assertEqual(self.disable(), 0)
        self.assertEqual(
            self.word("d2aspare").value,
            0xA5A5A5A5 | ISOLATION,
        )
        self.assertEqual(
            self.word("pllctl0").value,
            0x12345678 | VDDF_POWER_DOWN | VDDH_POWER_DOWN,
        )
        self.assertEqual(self.word("pll_write_count").value, 2)

    def test_second_pll_update_uses_a_fresh_volatile_read(self) -> None:
        injected = 0x00800000
        self.reset(
            revision=0,
            d2aspare=0,
            pllctl0=1,
            injected_pll_bits=injected,
        )

        self.assertEqual(self.disable(), 0)
        self.assertEqual(self.word("pllctl0").value, 0xC0800001)
        pll_reads = [
            value
            for event, address, value in self.trace()
            if event == self.READ and address == PLLCTL0
        ]
        self.assertEqual(pll_reads, [1, 0x80800001])

    def test_randomized_stock_model(self) -> None:
        generator = random.Random(0x0048009E)

        for _ in range(1000):
            revision = generator.getrandbits(32)
            d2aspare = generator.getrandbits(32)
            pllctl0 = generator.getrandbits(32)
            primask = generator.getrandbits(32)
            self.reset(revision, d2aspare, pllctl0, primask)

            self.assertEqual(self.disable(), 0)
            self.assertEqual(
                self.word("d2aspare").value,
                (
                    d2aspare | ISOLATION
                    if (revision & 0xFF) >= 0x22
                    else d2aspare
                ) & UINT_MASK,
            )
            self.assertEqual(
                self.word("pllctl0").value,
                pllctl0 | VDDF_POWER_DOWN | VDDH_POWER_DOWN,
            )
            self.assertEqual(self.word("restored_primask").value, primask)
            self.assertEqual(self.word("pll_write_count").value, 2)

    def test_sources_are_review_pinned(self) -> None:
        self.assertEqual(
            hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
            "7c2aae6442d99c67aaea64b687f1dabb8446df4907acc7dca53d992eaeeea695",
        )
        self.assertEqual(
            hashlib.sha256(FIXTURE.read_bytes()).hexdigest(),
            "9ed95f2d4eeb873530b00dafb22400fbd2a4a9657780f38bc4bea2f2e5096c5e",
        )


if __name__ == "__main__":
    unittest.main()
