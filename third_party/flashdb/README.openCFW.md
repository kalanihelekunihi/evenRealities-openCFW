...
default-KV node arrays and 20 of 21 initialized default values from the
authenticated IAR initialized-data record, plus the database mutex callbacks,
first-party magic/reset policy, and exact NOR callback return conventions.
The runtime-backed `kvbooCount` default, application structure semantics, and
golden external-flash oracle remain unresolved. In particular, do not copy the
